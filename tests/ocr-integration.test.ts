...
test("OCR integration uses Aliyun structured APIs for supplier documents with PDF fallback", () => {
  ...
  assert.match(service, /return await recognizeAliyunSupplierContract\(fileBuffer, settings\)/);
  ...
  assert.doesNotMatch(service, /withAliyunOcrRetry\("ALIYUN_RECOGNIZE_GENERAL_STRUCTURE"/);
