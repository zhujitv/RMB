...
test("OCR settings no longer expose customs declaration PDF text test entry points", () => {
  ...
  assert.doesNotMatch(settingsModule, /选择报关单 PDF 测试整单解析/);
  assert.doesNotMatch(settingsModule, /仅测试 PDF 文本解析，不调用 OCR，不保存业务数据。/);
});
...
test("settings module exposes OCR configuration without leaking secrets", () => {
  ...
  assert.doesNotMatch(settingsModule, /报关单识别模式/);
  assert.doesNotMatch(settingsModule, /自动模式/);
  assert.doesNotMatch(settingsModule, /严格结构化模式/);
  assert.doesNotMatch(settingsModule, /手工模式/);
  ...
  assert.match(settingsModule, /增值税发票及采购合同结构化识别需要 AccessKey ID \/ AccessKey Secret。报关单 OCR 已停用，不再提供相关配置。/);
...
test("OCR integration uses Aliyun structured APIs for supplier documents with PDF fallback", () => {
  ...
  assert.match(service, /supplierDocumentOcrBuffer/);
  assert.match(service, /rasterizeFirstPdfPageForOcr\(buffer\)/);
  assert.match(service, /supplierDocumentOcrSettings/);
  assert.match(service, /timeoutMs: Math\.max\(settings\.timeoutMs, 30_000\)/);
  assert.match(service, /return await recognizeAliyunVatInvoice\(ocrBuffer, ocrSettings, \{ maxAttempts: 1 \}\)/);
  assert.match(service, /return await recognizeAliyunSupplierContract\(ocrBuffer, ocrSettings, \{ maxAttempts: 1 \}\)/);
