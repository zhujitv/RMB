...
const componentHelpers = readFileSync("app/modules/sales-execution/purchase-order-price-correction-helpers.ts", "utf8");
...
test("purchase execution UI exposes price correction request and review feedback", () => {
  ...
  assert.match(componentSource, /采购价格更正申请/);
  assert.match(componentSource, /待管理员审核/);
  assert.match(componentSource, /title=\{reasonUnavailable \|\| undefined\}/);
  assert.match(componentSource, /disabled=\{busy \|\| !available\}/);
  assert.match(componentSource, /已有付款记录。审核通过后系统会保留原付款流水/);
  assert.match(componentSource, /结算更正凭证/);
  assert.match(componentSource, /工厂采购单确认接受后，才可以申请采购价格更正/);
  assert.match(componentSource, /通过/);
  assert.match(componentSource, /驳回/);
  assert.match(componentSource, /correction\.status === "PENDING" && canReview/);
  assert.match(componentSource, /const expectedStatus = action === "APPROVE" \? "APPROVED" : "REJECTED"/);
  assert.match(componentSource, /String\(result\.correction\?\.status \|\| ""\) !== expectedStatus/);
  assert.match(componentSource, /审核结果未生效/);
