...
test("logistics expense approval works at bill level and groups invoice emails by supplier", () => {
  ...
  assert.match(backend, /海运费、ENS费、保险费及所有 USD 费用统一归入“海运费发票”上传。/);
  assert.match(backend, /其他 CNY 物流费用可合并为“拖车及其他费用合并发票”上传。/);
  ...
  assert.match(logisticsBillConvergenceMigration, /LogisticsBill is the only workflow-state source/);
  ...
  assert.doesNotMatch(approveLogisticsExpenseBillRowsSource, /tx\.logisticsExpense\.updateMany\(\{[\s\S]*auditStatus: "审核通过"/);
  ...
  assert.match(backend, /prisma\.logisticsBill\.updateMany\(\{[\s\S]*auditStatus: "审核通过"/);
  assert.doesNotMatch(backend, /prisma\.logisticsExpense\.updateMany\(\{[\s\S]*auditStatus: "审核通过"/);
  ...
  assert.doesNotMatch(reviewLogisticsExpenseBillsSource, /notifyLogisticsSupplierInvoiceBills[\s\S]*prisma\.\$transaction/);
  assert.doesNotMatch(approveLogisticsExpenseBillRowsSource, /tx\.logisticsExpense\.update\(/);
  assert.doesNotMatch(approveLogisticsExpenseBillRowsSource, /include: includeLogisticsExpenseRelations/);
...
test("logistics invoice upload is grouped by required invoice categories", () => {
  ...
  assert.match(backend, /TRUCKING_OTHER_USD_BLOCK_MESSAGE = "USD 不允许出现在拖车及其他费用合并发票"/);
  assert.match(backend, /logisticsInvoiceGroupForExpense/);
  assert.match(backend, /logisticsInvoiceGroupsForExpenses/);
  assert.match(backend, /normalizedInvoiceCurrency\(expense\.currency\) === "USD"/);
  ...
  assert.match(backend, /const targetRows = rows\.filter\(\(row\) => logisticsInvoiceExpenseMatchesGroup\(row, invoiceGroup\)\)/);
  assert.match(logisticsModule, /logisticsInvoiceGroupForExpense\(item\)\?\.key === group\.key/);
  assert.match(logisticsInvoiceUsdGroupingMigration, /logistics_invoice_notification_template/);
  assert.match(logisticsInvoiceUsdGroupingMigration, /所有 USD 费用统一归入“海运费发票”/);
  assert.match(logisticsInvoiceUsdGroupingMigration, /LIKE '%ENS费%拖车及其他费用合并发票%'/);
...
test("draft logistics expense bills can be submitted for review", () => {
  ...
  assert.match(submitLogisticsExpenseBillSource, /prisma\.logisticsBill\.update/);
  assert.doesNotMatch(submitLogisticsExpenseBillSource, /prisma\.logisticsExpense\.updateMany/);
