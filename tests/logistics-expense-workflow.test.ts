...
test("logistics expense form supports positive applied quantity", () => {
  ...
  assert.match(logisticsModule, /step="1"/);
...
test("logistics suppliers can edit price and quantity before audit approval", () => {
  ...
  assert.match(workspaceStyles, /\.inlineQuantityInput[\s\S]*width: 72px/);
  ...
  assert.match(backend, /LOGISTICS_EXPENSE_BATCH_CONTAINER_COUNT_INVALID/);
