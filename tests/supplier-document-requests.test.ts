...
test("supplier document upload is not blocked when OCR task creation fails", () => {
  ...
  assert.match(service, /const completedTask = await runNonCriticalTask\("产品供应商回传资料OCR识别"/);
  assert.match(service, /return runSupplierDocumentOcrTask\(ocrTask\.id\)/);
  assert.match(service, /completedTask\?\.status === "OCR识别失败，需人工核对"/);
