...
} from "../lib/customs-declaration-parser.js";
...
test("parser source does not reference bundled fixture documents", async () => {
  const source = await fs.readFile(new URL("../lib/customs-declaration-parser.js", import.meta.url), "utf8");
  const forbidden = [
    "05-versions" + "-space",
    ["test", "data"].join("/"),
    "sample " + "pdf",
  ...
  assert.doesNotMatch(source, /import\(["']pdf-parse["']\)|require\(["']pdf-parse["']\)/);
  assert.match(source, /pdf-parse\/lib\/pdf-parse\.js/);
});
