...
test("product supplier cost payment vouchers are scoped away from logistics fees", () => {
  ...
  assert.match(costsModule, /surfaceClassName=\{styles\.paymentVoucherModal\}/);
  assert.match(costsModule, /method: "HEAD"/);
  assert.match(costsModule, /文件暂时无法预览，请下载查看。/);
  assert.match(costsModule, /paymentVoucherDownloadUrl\(cost, "attachment"\)/);
  assert.match(costsModule, /download=1/);
