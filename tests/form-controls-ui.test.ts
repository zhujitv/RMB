...
test("user permissions reuse unified card multi-select options", () => {
  ...
  assert.match(workspaceStyles, /\.userEditPanel \{[\s\S]*max-width: 960px;/);
  assert.match(workspaceStyles, /\.permissionModeCards \{[\s\S]*grid-template-columns: repeat\(2, minmax\(0, 1fr\)\);/);
  assert.match(workspaceStyles, /\.dataScopeCardGrid \{[\s\S]*grid-template-columns: repeat\(4, minmax\(0, 1fr\)\);/);
