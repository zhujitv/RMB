...
Page({
  data: { id: "", loading: true, error: "", quotation: null, version: null, canEdit: false, canOperate: false },
  ...
  async load() {
    ...
    try {
      ...
      this.setData({ quotation: { ...quotation, statusDisplay: STATUS_LABELS[quotation.status] || quotation.status }, version, canEdit: canWrite && ["DRAFT", "SENT", "REJECTED"].includes(quotation.status), canOperate: canWrite && ["DRAFT", "SENT"].includes(quotation.status) });
    } catch (error) { this.setData({ error: error.message }); }
    finally { this.setData({ loading: false }); }
  },
  edit() { wx.navigateTo({ url: `/pages/quotation-form/index?id=${encodeURIComponent(this.data.id)}` }); },
  operate() { wx.navigateTo({ url: `/pages/quotation-actions/index?id=${encodeURIComponent(this.data.id)}` }); },
});
