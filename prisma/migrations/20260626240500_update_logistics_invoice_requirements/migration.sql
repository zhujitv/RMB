...
WHERE "key" = 'logistics_invoice_notification_template'
  AND COALESCE("value"->>'invoiceRequirements', '') NOT LIKE '%所有 USD 费用统一归入“海运费发票”%'
  AND (
    COALESCE("value"->>'invoiceRequirements', '') LIKE '%海运费必须分别开票上传%'
    OR COALESCE("value"->>'invoiceRequirements', '') LIKE '%保险费%拖车及其他费用合并发票%'
  );
