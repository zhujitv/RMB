...
GitHub Runner 的 SSH 与 SCP 固定使用 OpenSSH 的 `ConnectionAttempts=3`，只在 TCP 连接建立前进行有限重试。认证失败、主机指纹错误、权限错误以及远端命令执行中断都不会自动重放，避免在执行状态不明确时重复构建或切换；此时应先复核服务器状态，再重新运行工作流。
...
服务重启后除了检查本机和公网 `/api/health` 的目标 SHA，还会实际加载 `/api/auth/me?basic=1`。该认证探针只接受正常的 `200` 或未登录状态 `401`，模块加载失败返回的 `5xx` 会触发自动回滚，避免“健康接口正常但用户无法登录”。
