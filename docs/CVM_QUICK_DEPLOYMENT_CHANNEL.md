...
GitHub Runner 首次连接腾讯云以及上传校验过的小型 bootstrap 时，如 SSH 传输层返回连接失败，最多重试 3 次，间隔 5 秒和 15 秒。这两步都发生在远端部署脚本启动之前。真正的远端部署命令不会因连接中断而自动重放，避免在执行状态不明确时重复构建或切换；此时应先复核服务器状态，再重新运行工作流。
