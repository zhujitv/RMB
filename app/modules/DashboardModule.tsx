...
export function DashboardModule({ onOpenModule }: { onOpenModule?: (target: DashboardOpenTarget, keyword?: string) => void }) {
  ...
  const periodMetrics = [
    ...
    { label: "新单预计毛利率", value: formatPercent(totals.expectedGrossMargin), note: "成本不完整订单会在风险区提示", tone: styles.metricBlue, target: "profit" as const },
  ...
  const riskMetrics = [
    ...
    { label: "亏损订单", value: `${Number(totals.negativeMarginOrders || 0)} 单`, note: "当前预计毛利小于 0", tone: Number(totals.negativeMarginOrders || 0) > 0 ? styles.metricRed : styles.metricGreen, target: "profit" as const },
    { label: "低毛利订单", value: `${Number(totals.lowMarginOrders || 0)} 单`, note: "当前预计毛利率低于 8%", tone: Number(totals.lowMarginOrders || 0) > 0 ? styles.metricOrange : styles.metricGreen, target: "profit" as const },
