...
function OfflineResultDialog({ executionId, load, allLoads, order, allocations, onSaved, onClose }: {
  ...
  function validate() {
    ...
    if (!load.loadingDate) return "柜总单缺少装柜日期，请先重新建立草稿";
    let differs = false;
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const response = await apiJson<{ message?: string }>(`/api/sales-executions/${encodeURIComponent(executionId)}/purchase-orders/${encodeURIComponent(order.id)}/offline-loading-result`, {
        method: "POST",
        body: JSON.stringify({ containerLoadId: load.id, expectedRevision: load.revision, loadingDate: load.loadingDate?.slice(0, 10), channel, supplierContact: supplierContact.trim(), reason, reasonDetail: reasonDetail.trim(), items: lines.map((line) => ({ purchaseOrderItemId: line.item.id, loadedQuantity: String(values[line.item.id] || "").trim() })) }),
      });
