...
export function ContainerLoadCard({ executionId, load, allLoads, orders, canManage, shippingStarted, onEdit, onSaved }: {
  ...
  return <article className={styles.card} data-status={load.status}>
    ...
    <div className={styles.facts}><div><span>{isBulkWarehouse ? "进舱日期" : "装柜日期"}</span><strong>{formatDate(load.loadingDate)}</strong></div><div><span>供应商</span><strong>{supplierCount} 家</strong></div><div><span>计划明细</span><strong>{load.allocations.length} 条</strong></div><div><span>单据版本</span><strong>V{load.revision}</strong></div></div>
    <div className={styles.toolbar}>{canManage && load.status === "DRAFT" ? <><button className={styles.secondaryButton} type="button" disabled={busy || shippingStarted} onClick={onEdit}>编辑草稿</button><button className={styles.button} type="button" disabled={busy || shippingStarted || !load.loadingDate || !load.allocations.length} onClick={() => mutate("open")}>开放供应商填报</button></> : null}{canManage && load.status === "OPEN" ? <button className={styles.button} type="button" disabled={busy || shippingStarted || !releaseReady} title={releaseReady ? undefined : "所有供应商实装/进舱结果确认后才能放行"} onClick={() => mutate("release")}>最终放行</button> : null}{canManage && (load.status === "DRAFT" || load.status === "OPEN") ? <button className={styles.dangerButton} type="button" disabled={busy || shippingStarted || pending.length > 0 || approvedIds.size > 0} onClick={() => mutate("void")}>作废该装运单</button> : null}</div>
