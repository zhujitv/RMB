...
export function ContainerLoadCard({ executionId, load, allLoads, orders, canManage, shippingStarted, onEdit, onSaved }: {
  ...
  const isBulkWarehouse = !load.containerNo;
  ...
  return <article className={styles.card} data-status={load.status}>
    <header className={styles.cardHeader}><div><p className={styles.eyebrow}>第 {load.sequenceNo} 个装运单</p><h4>{load.containerNo || "散货进舱（无柜号）"}</h4><p>{isBulkWarehouse ? "按进舱批次登记" : `${load.containerType || "柜型未填"}${load.sealNo ? ` · 封号 ${load.sealNo}` : ""}`}</p></div><span className={styles.status} data-status={load.status}>{containerLoadStatusLabel(load.status)}</span></header>
    <div className={styles.facts}><div><span>{load.status === "RELEASED" ? (isBulkWarehouse ? "实际进舱日期" : "实际装柜日期") : "预计日期"}</span><strong>{load.loadingDate ? formatDate(load.loadingDate) : "待最终放行确认"}</strong></div><div><span>供应商</span><strong>{supplierCount} 家</strong></div><div><span>计划明细</span><strong>{load.allocations.length} 条</strong></div><div><span>单据版本</span><strong>V{load.revision}</strong></div></div>
    <div className={styles.toolbar}>{canManage && load.status === "DRAFT" ? <><button className={styles.secondaryButton} type="button" disabled={busy || shippingStarted} onClick={onEdit}>编辑草稿</button><button className={styles.button} type="button" disabled={busy || shippingStarted || !load.allocations.length} onClick={() => mutate("open")}>开放供应商填报</button></> : null}{canManage && load.status === "OPEN" ? <button className={styles.button} type="button" disabled={busy || shippingStarted || !releaseReady} title={releaseReady ? undefined : "所有供应商实装/进舱结果确认后才能放行"} onClick={() => mutate("release")}>最终放行</button> : null}{canManage && (load.status === "DRAFT" || load.status === "OPEN") ? <button className={styles.dangerButton} type="button" disabled={busy || shippingStarted || pending.length > 0 || approvedIds.size > 0} onClick={() => mutate("void")}>作废该装运单</button> : null}</div>
