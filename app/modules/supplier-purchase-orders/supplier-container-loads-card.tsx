...
function SupplierContainerLoadCard({ canWrite, detail, load, disabled, onSaved }: {
  ...
  return <article className={styles.supplierCard} data-status={load.status}>
    <header className={styles.cardHeader}><div><p className={styles.eyebrow}>第 {load.sequenceNo} 个装运单</p><h4>{load.containerNo || "散货进舱（无柜号）"}</h4><p>{load.containerNo ? `${load.containerType || "柜型未填写"}${load.sealNo ? ` · 封号 ${load.sealNo}` : ""}` : "请填写本次实际进舱数量"}</p></div><span className={styles.status} data-status={load.status}>{containerLoadStatusLabel(load.status)}</span></header>
    <div className={styles.facts}><div><span>{load.status === "RELEASED" ? (load.containerNo ? "实际装柜日期" : "实际进舱日期") : "预计日期"}</span><strong>{load.loadingDate ? formatDate(load.loadingDate) : "最终放行时记录"}</strong></div><div><span>本单计划明细</span><strong>{state.allocations.length} 条</strong></div><div><span>填报状态</span><strong>{state.active ? containerLoadingResultStatusLabel(state.active.status) : load.status === "OPEN" ? "待填报" : "-"}</strong></div><div><span>任务版本</span><strong>V{load.revision}</strong></div></div>
