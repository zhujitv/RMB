        ...
        {form.preview ? <>
          ...
          <div className={styles.supplierDocumentTransitionTableWrap}><table className={styles.supplierDocumentTransitionTable}>
            ...
            >
              ...
              <td><input className={styles.supplierDocumentTransitionField} aria-label={`过渡结算第${index + 1}行品名`} value={item.productName || ""} disabled={Boolean(form.preview?.existing)} onChange={(event) => form.updateItem(index, { productName: event.target.value })} onClick={(event) => event.stopPropagation()} /></td>
              <td><input className={styles.supplierDocumentTransitionField} aria-label={`过渡结算第${index + 1}行数量`} inputMode="decimal" value={item.quantity || ""} disabled={Boolean(form.preview?.existing)} onChange={(event) => form.updateItem(index, { quantity: event.target.value })} onClick={(event) => event.stopPropagation()} /></td>
              <td><input className={styles.supplierDocumentTransitionField} aria-label={`过渡结算第${index + 1}行单位`} value={item.unit || ""} disabled={Boolean(form.preview?.existing)} onChange={(event) => form.updateItem(index, { unit: event.target.value })} onClick={(event) => event.stopPropagation()} /></td>
            </tr>)}</tbody>
          </table></div>
