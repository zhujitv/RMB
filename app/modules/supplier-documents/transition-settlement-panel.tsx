...
export function TransitionSettlementPanel({ cost, saving, form, onError }: {
  ...
  return (
    <section className={`${styles.supplierDocumentUploadCard} ${styles.supplierDocumentTransitionPanel}`} aria-label="历史过渡结算">
      ...
      <div className={`${styles.supplierDocumentUploadBody} ${styles.supplierDocumentTransitionBody}`}>
        ...
        {form.preview ? <>
          ...
          <div className={styles.supplierDocumentTransitionTableWrap}><table className={styles.supplierDocumentTransitionTable}>
            ...
            <tbody>{form.items.map((item, index) => <tr key={`${item.customsItemIndex}-${index}`}>
              <td><input className={styles.supplierDocumentTransitionCheckbox} aria-label={`选择第${index + 1}行报关商品`} type="checkbox" checked={Boolean(item.selected)} disabled={Boolean(form.preview?.existing)} onChange={(event) => form.updateItem(index, { selected: event.target.checked })} /></td>
              <td><input className={styles.supplierDocumentTransitionField} aria-label={`过渡结算第${index + 1}行品名`} value={item.productName || ""} disabled={Boolean(form.preview?.existing)} onChange={(event) => form.updateItem(index, { productName: event.target.value })} /></td>
              <td><input className={styles.supplierDocumentTransitionField} aria-label={`过渡结算第${index + 1}行数量`} inputMode="decimal" value={item.quantity || ""} disabled={Boolean(form.preview?.existing)} onChange={(event) => form.updateItem(index, { quantity: event.target.value })} /></td>
              <td><input className={styles.supplierDocumentTransitionField} aria-label={`过渡结算第${index + 1}行单位`} value={item.unit || ""} disabled={Boolean(form.preview?.existing)} onChange={(event) => form.updateItem(index, { unit: event.target.value })} /></td>
            </tr>)}</tbody>
          </table></div>
