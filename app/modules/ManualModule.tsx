...
const SECTIONS: ManualSection[] = [
  ...
  },
  {
    ...
    no: "06",
    title: "物流信息",
    summary: "物流信息是物流模块唯一入口，集中维护物流状态、首程运输事实数据、报关资料、物流费用录入与审核；运输监控作为物流信息下的在途海运监控入口。",
    steps: ["选择车辆运输、快递运输、多式联运或散货进舱，并填写对应运输/进舱信息。", "系统生成退税资料所需运输备注。", "上传报关单、放行通知书、报关委托书。", "报关单 PDF 上传成功后，系统自动识别报关单号和申报日期，并同步到退税资料。", "在页面底部“物流费用录入与审核”区域录入供应商费用、查看月结汇总和供应商费用明细。", "对待审核费用执行合并审核 / 批量审核，并按发票分组上传、确认发票和维护付款状态。", "需要对账时可导出对账单。"],
    notes: ["左侧菜单不再单独显示“物流费用审核”，所有费用审核流程都从物流信息进入。", "物流资料上传后自动归属到订单，并进入退税资料统计。", "报关单只保留一个当前有效 PDF，重新上传会替换旧文件。", "如果识别失败，不影响文件上传，可在退税资料详情中手工填写或重新识别。"],
    entry: "物流信息",
  },
  {
    ...
    notes: ["管理员、业务员和物流资料录入员可查看系统内所有已创建的大掌櫃海运跟踪；物流供应商和产品供应商账号不可见。", "运输监控默认只显示未到港 / 未完成的在途业务，已到港或已完成记录可通过筛选查看。", "一张 Master B/L 在系统生命周期内只创建一次大掌櫃 Tracking；柜号只用于本地关联查询，避免重复消耗 Credit。", "后台页面优先显示中文船公司、港口、状态和跟踪方式，原始英文数据仍保留在系统中。"],
    entry: "物流信息 → 运输监控",
  },
  {
    id: "tax-refunds",
  ...
  },
  {
    ...
    notes: ["公司资料会影响登录页品牌、工作台侧边栏、首页欢迎语、Logo 和页脚版权等系统展示。", "汇率影响所有外币折人民币金额。", "客户资料中的业务员和提成比例会影响利润与提成统计。", "用户权限决定能看到哪些模块。", "产品供应商账号只用于资料回传；物流类供应商账号用于物流资料、费用和发票相关操作。", "大掌櫃关闭后，物流信息和运输监控不显示相关创建、同步和查看入口。"],
    entry: "系统设置",
  },
  {
    id: "security",
    ...
    notes: ["业务员、财务、物流供应商、产品供应商和管理员看到的模块不同，不要借用他人账号操作。", "所有业务附件上传入口统一只支持 PDF，单个文件最大 5MB，选择文件后自动上传并显示进度。", "文件预览、下载、删除均受权限控制。", "登录失败和系统错误会记录安全日志，但不会在页面暴露数据库、密钥或部署细节。", "含脚本、自动打开动作、嵌入文件等主动内容的 PDF 会被系统拒绝。"],
  ...
  },
  {
    ...
    steps: ["所有收款、成本、物流、退税资料必须关联订单。", "成本未确认时，不应进入最终利润结算。", "物流费用审核不再单独作为菜单入口，请从物流信息进入费用录入与审核。", "总体完整度不足 100%，不允许提交退税。", "任何关键修改都必须写入操作日志。", "不要上传来源不明或带主动内容的 PDF 附件。"],
