...
const SECTIONS: ManualSection[] = [
  {
    ...
    no: "01",
    title: "系统定位",
    summary: "本系统用于管理外贸业务从应收订单、客户回款、成本录入、国内物流资料、退税资料收集，到退税提交和资料归档的全过程。",
  ...
  },
  {
    ...
    steps: ["建立应收订单", "登记客户回款", "录入成本费用", "录入国内物流信息", "汇总退税资料", "检查总体完整度", "提交退税", "自动归档", "后续在档案中查询"],
    notes: ["每个环节都要关联同一个订单。"],
    entry: "左侧业务菜单",
    defaultOpen: true,
  },
  {
    ...
    steps: ["选择客户并核对客户名称。", "填写订单号、提单号、币种、汇率和最终应收金额。", "确认付款条款、预计发货日期、提单日期、到期日和订单状态。"],
  ...
  },
  {
    ...
    no: "05",
    title: "成本管理",
    summary: "成本管理用于记录每票订单产生的工厂货款、拖车费、报关费、港杂费、海运费、手续费、佣金和其它费用。",
  ...
  },
  {
    ...
    steps: ["填写运输方式、起运地、到达地、运输货物名称、起运日期、车牌号或快递单号。", "系统生成出口发票备注。", "上传报关单、放行通知书、报关委托书。"],
    notes: ["物流资料上传后自动归属到订单，并进入退税资料统计。"],
    entry: "国内物流信息",
  },
  {
    id: "tax-refunds",
    no: "07",
    title: "退税资料",
    summary: "退税资料模块按订单汇总出口资料、报关资料、工厂资料、物流资料和国内物流信息。",
    steps: ["查看总体完整度。", "进入查看资料补齐缺失 PDF。", "确认资料完整后提交退税。"],
    notes: ["列表只保留总体完整度，分类明细在查看资料中处理。", "总体完整度不足 100% 不允许提交退税。"],
    entry: "退税资料",
  },
  {
    id: "archive",
    ...
    notes: ["归档不是删除。", "归档后的订单默认不再显示在当前退税资料、成本管理、国内物流当前业务和经营待处理提醒中。", "退税归档只代表资料链完成，不代表货款已经收齐。"],
    entry: "退税资料 → 退税档案",
    defaultOpen: true,
  },
  {
  ...
  },
  {
    ...
    no: "10",
    title: "系统设置",
    summary: "管理员维护汇率、客户、供应商、用户权限和操作日志。",
    steps: ["维护客户资料和负责业务员。", "维护供应商类型和默认物流供应商。", "维护用户角色、权限和账号状态。", "查看操作日志追溯关键修改。"],
    notes: ["汇率影响所有外币折人民币金额。", "客户资料中的业务员和提成比例会影响利润与提成统计。", "用户权限决定能看到哪些模块。"],
    entry: "系统设置",
  },
  {
    id: "daily-notes",
    ...
    steps: ["所有收款、成本、物流、退税资料必须关联订单。", "成本未确认时，不应进入最终利润结算。", "总体完整度不足 100%，不允许提交退税。", "任何关键修改都必须写入操作日志。"],
    notes: ["已提交退税后自动归档。", "归档不是删除，只是放入档案室。", "收款未完成不影响退税归档。", "退税归档不等于订单关闭。"],
    entry: "全平台",
  },
];
