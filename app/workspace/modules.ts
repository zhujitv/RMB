...
export const MODULE_DESCRIPTORS: Record<string, ModuleDescriptor> = {
  ...
  payments: {
    ...
    stage: "ready",
    migrationNotes: ["支持后端分页列表、登记、编辑、确认到账和删除", "收款状态保存继续由后端权限校验", "正式回款统计仅计入已到账收款"],
  },
  costs: {
    key: "costs",
    label: "成本管理",
    ...
    stage: "ready",
    migrationNotes: ["支持分页列表、当前/归档业务筛选、普通成本登记、编辑、删除和资料维护", "物流信息内保留按票录入物流费用入口", "成本资料维护支持工厂合同、工厂发票和物流发票上传/预览/下载/删除"],
  },
  profit: {
    key: "profit",
    label: "利润分析",
  ...
  taxRefund: {
    ...
    stage: "ready",
    migrationNotes: ["支持当前资料和退税档案列表", "支持按需资料详情、缺失资料跳转、报关单信息手工维护、文件上传删除、资料包下载、提交归档和取消归档", "提交归档继续由后端强制校验完整度"],
  },
  reports: {
    key: "reports",
    label: "报表中心",
  ...
  settings: {
    ...
    stage: "ready",
    migrationNotes: ["支持客户、供应商、用户、权限矩阵和汇率基础设置", "支持客户清关资料自动通知配置", "操作日志按需分页查看", "布尔字段统一显示开启/关闭"],
  },
};
