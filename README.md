...
采购价格录入错误时可以发起批量采购价格更正：一次勾选多条产品行，分别填写正确单价并填写统一原因。管理员对整批一次通过或驳回；系统逐行保留差额凭证，但最终应付已确认时整批只推进一个结算版本，并同时计算增加金额、扣减金额和净差额。历史原价、原结算与全部审核记录均不覆盖。
...
- 产品供应商回传文件后，系统自动执行 OCR 辅助识别和内容校验，但 OCR 结果不直接作为最终业务依据
- 正常订单和过渡订单的合同商品表均允许管理员修改品名、数量、单位、单价、总价，并可新增或删除商品行
- 发票商品表同样允许上述五项人工修正及增删行；保存后系统按人工结果重新执行合同匹配
- 原始 OCR 快照禁止覆盖；人工版本保存版本号、操作人、时间和前后差异。合同与发票必须人工核查确认后，资料回传任务才可完成
...
产品供应商资料 OCR 校验规则：
- OCR 是人工核对的预填工具，人工保存的商品表是后续合同生成、发票匹配和递交依据
- 可编辑字段限定为品名、数量、单位、单价和总价；支持新增、删除商品行
...
- 原始 OCR 结果和人工有效版本分开保存；原始值不可覆盖，人工值采用版本冲突保护并写入操作日志
