...
单一工厂订单可使用整单默认工厂；多工厂订单按产品拆分。工厂使用模糊查找，采购单价允许留空等待供应商首次回复时回填。正式下发后，销售内容和工厂分配锁定，并按工厂生成独立采购单和通知。

### 工厂采购、生产与结算

产品供应商在工厂采购单门户查看本工厂采购单，可回填缺失采购价、接受交期、提出新交期或拒绝。新交期由内部人员确认；被拒采购单保留作废记录，并可重新选厂后单独下发。

每家已接受且满足预付款条件的工厂可独立开始生产，不必等待其它工厂。生产完成必须由供应商确认，内部人员随后登记不可修改的实际交付日期。所有有效采购单均已接受、完工并登记实际交付后，销售执行才可进入发货并生成关联应收订单草稿。当前版本不包含质检模块。
...
“已实际发货”以已登记实际发货日期或实际发货金额为准。未发货订单仍保留预计毛利金额用于预测，但利润率显示“未发货”，且不进入低毛利、亏损订单和利润率排行；客户款项未收齐时，已实现毛利率显示 `--`。
...
```text
20260812140000_procurement_database_invariant_corrections
```

该迁移在既有报价、销售执行和采购迁移之后补强业务主体、供应商响应、交期、生产、付款、费用和结算一致性约束。生产执行前必须先备份并检查 migration 状态，禁止使用 `prisma db push` 替代正式 migration。
