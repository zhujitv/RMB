...
系统已在左侧导航新增“操作手册”入口，并内置操作手册页面。操作手册内容由 `app/modules/ManualModule.tsx` 维护，后端角色菜单已加入 `manual` 菜单权限。

当前生产发布以 GitHub 到 Vercel 的集成链路为准：
...
- 推送到 `main` 后，由 Vercel Git Integration 自动构建并发布对应 commit。
- GitHub Actions 的 `CI` workflow 运行 `npm run verify:ci`，只负责代码校验，不负责生产发布。
- `.github/workflows/vercel-deploy.yml` 只保留为手动说明入口，不调用 Vercel CLI，也不依赖 `VERCEL_TOKEN`。
- 如需数据库结构变更，必须在上线前或上线窗口单独执行 `npm run db:deploy`；Vercel 构建只会生成 Prisma Client，不会替代生产迁移。
- 工作区存在无关文件时，只 stage 本次变更文件，避免把临时文档、输出目录或调试文件带入部署 commit。
- 如果 Vercel 没有生成部署记录，优先检查 Vercel 项目的 GitHub 连接、生产分支和最新 commit 是否已推送到 `main`。
