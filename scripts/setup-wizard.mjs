...
async function askPassword() {
  while (true) {
    const password = await askRequired("设置初始管理员密码（至少8位，包含大小写字母，至少2个英文字母）");
...
async function collectDatabaseUrl() {
  const existing = await ask("如果已有完整 PostgreSQL DATABASE_URL，请粘贴；没有则直接回车逐项填写");
  ...
  const password = await askRequired("数据库密码");
  const sslMode = await ask("SSL 模式 require / prefer / disable", "require");

  return `postgresql://${encodeConnectionPart(user)}:${encodeConnectionPart(password)}@${host}:${port}/${encodeConnectionPart(database)}?sslmode=${sslMode}`;
}
...
async function collectR2() {
  ...
  return {
    ...
    R2_ACCESS_KEY_ID: await askRequired("R2 Access Key ID"),
    R2_SECRET_ACCESS_KEY: await askRequired("R2 Secret Access Key"),
...
async function collectEmail() {
  ...
  return {
    RESEND_API_KEY: await askRequired("Resend API Key"),
...
async function collectRedis() {
  const enabled = isYes(await ask("是否配置 Upstash Redis 分布式限流？小团队可先不配置。y/N"), false);
  if (!enabled) {
    return {
  ...
  return {
    ...
    UPSTASH_REDIS_REST_TOKEN: await askRequired("Upstash Redis REST Token"),
    RATE_LIMIT_REDIS_REST_URL: "",
    RATE_LIMIT_REDIS_REST_TOKEN: "",
...
async function main() {
  ...
  const cronSecret = await ask("CRON_SECRET 随机密钥", randomSecret());
  ...
  writeFileSync(targetFile, renderEnv(values), "utf8");
