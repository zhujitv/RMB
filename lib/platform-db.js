...
export const DOMESTIC_LOGISTICS_FINANCE_STATUSES = ["DRAFT", "PENDING", "CONFIRMED", "REJECTED"];
export const DOMESTIC_LOGISTICS_STATUS_LABELS = {
  DRAFT: "草稿",
  PENDING: "待财务审核",
  CONFIRMED: "财务已确认",
  REJECTED: "已驳回",
...
export const ROLE_SCOPE_TEXT = {
  ...
  [LOGISTICS_OPERATOR_ROLE]: "仅可录入本供应商关联订单的国内物流信息",
  查看者: "只读权限",
};
...
function serializeDomesticLogisticsInfo(row) {
  ...
  return {
    ...
    responsibleSupplierId: row.responsibleSupplierId || "",
    responsibleSupplierName: row.responsibleSupplier?.supplierName || "",
    temporarySupplierName: row.temporarySupplierName || "",
    submittedByUserId: row.submittedByUserId || "",
    submittedByName: row.submittedBy?.name || "",
    submittedAt: row.submittedAt,
    submitterRole: row.submitterRole || "",
    financeStatus: row.financeStatus || "PENDING",
    financeStatusLabel: DOMESTIC_LOGISTICS_STATUS_LABELS[row.financeStatus] || row.financeStatus || "待财务审核",
    financeConfirmedBy: serializeUser(row.financeConfirmedBy),
    financeConfirmedByName: row.financeConfirmedBy?.name || "",
    financeConfirmedAt: row.financeConfirmedAt,
...
export function taxDocumentCompleteness(order = {}) {
  ...
  const domesticLogisticsComplete = Boolean(
    ...
    && domesticLogisticsInfo.financeStatus === "CONFIRMED"
  ...
  const domesticLogisticsMissing = domesticLogisticsComplete ? [] : [{
    ...
    label: "国内物流信息",
    financeStatus: domesticLogisticsInfo?.financeStatus || "MISSING",
  }];
  ...
  const total = TAX_EXPORT_DOCUMENT_TYPES.length + domesticLogisticsTotal + supplierTotal + logisticsTotal;
  const completed = exportCompleted + domesticLogisticsCompleted + supplierCompleted + logisticsCompleted;
  const factory = {
  ...
  return {
    ...
    total,
    completed,
    missingTypes: [...exportMissing, ...domesticLogisticsMissing.map((item) => item.documentType), ...supplierMissing.map((item) => item.documentType), ...logisticsMissing.map((item) => item.documentType)],
    missingLabels,
...
function emptyTaxRefundCompleteness() {
  ...
  return {
    complete: false,
    total: TAX_EXPORT_DOCUMENT_TYPES.length + 1 + supplierTotal + TAX_REFUND_LOGISTICS_INVOICE_REQUIREMENTS.length,
    completed: 0,
    missingTypes: [],
    missingLabels: [],
...
export function includeOrderRelations() {
  return {
    ...
    domesticLogisticsInfos: {
      where: { deletedAt: null },
      include: { responsibleSupplier: true, submittedBy: true, financeConfirmedBy: true },
...
export async function saveUser(request, actor, input, id = null) {
  ...
  if (role === LOGISTICS_OPERATOR_ROLE) {
    if (!supplierId) throw codedError("物流资料录入员必须选择关联供应商。", 400, "SUPPLIER_REQUIRED");
    const supplier = await assertDomesticLogisticsSupplier(supplierId);
    boundSupplierId = supplier.id;
  }
...
function domesticLogisticsCostWhereForSupplier(supplierId) {
  return {
    deletedAt: null,
    supplierId,
    OR: [
      { costType: { in: LOGISTICS_COST_TYPES } },
      { costType: { in: TAX_REFUND_LOGISTICS_INVOICE_COST_TYPES } },
      { supplier: { is: { supplierType: { in: DOMESTIC_LOGISTICS_SUPPLIER_TYPES } } } },
    ],
  };
}
...
function qualifyingDomesticLogisticsSupplierIds(order = {}) {
  return [...new Set((order.costs || [])
    .filter((cost) => (
      !cost.deletedAt
      && cost.supplierId
      && (
        isLogisticsCostType(cost.costType)
        || TAX_REFUND_LOGISTICS_INVOICE_COST_TYPES.includes(cost.costType)
        || DOMESTIC_LOGISTICS_SUPPLIER_TYPES.includes(cost.supplier?.supplierType || "")
      )
    ))
    .map((cost) => cost.supplierId))];
}

function autoDomesticLogisticsSupplierId(order = {}) {
  const ids = qualifyingDomesticLogisticsSupplierIds(order);
  return ids.length === 1 ? ids[0] : "";
}

function actorDomesticLogisticsSupplierId(actor) {
...
function canReadDomesticLogisticsOrder(actor, order = {}) {
  ...
  const supplierId = actorDomesticLogisticsSupplierId(actor);
  if (!supplierId) return false;
  const info = (order.domesticLogisticsInfos || [])[0];
  if (info?.responsibleSupplierId) return info.responsibleSupplierId === supplierId;
  return qualifyingDomesticLogisticsSupplierIds(order).includes(supplierId);
}
...
function domesticLogisticsStatusText(info = null) {
  if (!info) return "未提交";
  if (info.financeStatus === "CONFIRMED") return "已确认";
...
function serializeDomesticLogisticsOrder(order) {
  ...
  const supplierId = info?.responsibleSupplierId || autoDomesticLogisticsSupplierId(order);
  const supplier = supplierId
    ? (order.costs || []).find((cost) => cost.supplierId === supplierId)?.supplier || info?.responsibleSupplier
    : null;
  const candidateSuppliers = Object.values((order.costs || []).reduce((acc, cost) => {
    if (!cost.supplierId || !cost.supplier) return acc;
    if (!qualifyingDomesticLogisticsSupplierIds(order).includes(cost.supplierId)) return acc;
    acc[cost.supplierId] ||= {
      id: cost.supplierId,
      supplierName: cost.supplier.supplierName,
      supplierType: cost.supplier.supplierType,
      allowDomesticLogisticsEntry: Boolean(cost.supplier.allowDomesticLogisticsEntry),
    };
    return acc;
  }, {}));
  return {
    ...
    destinationPort: order.expectedArrivalDate ? "" : "",
    responsibleSupplierId: supplierId || "",
    responsibleSupplierName: supplier?.supplierName || info?.responsibleSupplierName || "",
    temporarySupplierName: info?.temporarySupplierName || "",
    candidateSuppliers,
    logisticsStatus: info ? (info.remarkText ? "已提交" : "未完成") : "未提交",
    financeStatus: info?.financeStatus || "MISSING",
    financeStatusLabel: info?.financeStatusLabel || "未提交",
...
export async function listDomesticLogisticsOrders(query, actor) {
  ...
  const supplierId = actorDomesticLogisticsSupplierId(actor);
  if (actor?.role === LOGISTICS_OPERATOR_ROLE) await assertDomesticLogisticsSupplier(supplierId);
  const andConditions = [];
  if (keyword) {
    andConditions.push({
      OR: [
  ...
  if (supplierId) {
    andConditions.push({
      OR: [
        { domesticLogisticsInfos: { some: { responsibleSupplierId: supplierId, deletedAt: null } } },
        { costs: { some: domesticLogisticsCostWhereForSupplier(supplierId) } },
      ],
    });
  }
  if (actor?.role === "业务员") {
...
export async function saveDomesticLogisticsInfo(request, actor, input, id = null) {
  ...
  if (actor.role === "财务") {
    throw codedError("财务只能审核国内物流信息，不能录入或修改物流内容。", 403, "FINANCE_CANNOT_EDIT_DOMESTIC_LOGISTICS");
  }
  ...
  if (before?.financeStatus === "CONFIRMED" && actor.role !== "管理员") {
    throw codedError("财务已确认的物流信息已锁定，请联系管理员解锁后修改。", 400, "DOMESTIC_LOGISTICS_LOCKED");
  }
  const transportType = DOMESTIC_LOGISTICS_TRANSPORT_TYPES.includes(input.transportType) ? input.transportType : "TRUCK";
  let responsibleSupplierId = "";
  let temporarySupplierName = "";
  const supplierMode = nonEmpty(input.supplierMode || input.supplier_mode || "existing");
  if (actor.role === LOGISTICS_OPERATOR_ROLE) {
    responsibleSupplierId = actorDomesticLogisticsSupplierId(actor);
    if (!responsibleSupplierId) throw codedError("物流资料录入员未绑定供应商，不能录入国内物流信息。", 403, "SUPPLIER_REQUIRED");
  } else if (supplierMode === "temporary") {
    temporarySupplierName = requireText(input.temporarySupplierName, "临时物流供应商名称");
  } else if (supplierMode === "none") {
    responsibleSupplierId = "";
  } else {
    responsibleSupplierId = input.responsibleSupplierId
      ? requireText(input.responsibleSupplierId, "物流负责供应商")
      : (before?.responsibleSupplierId || autoDomesticLogisticsSupplierId(order));
  }
  if (responsibleSupplierId) {
    await assertDomesticLogisticsSupplier(responsibleSupplierId);
  }
  if (actor.role === LOGISTICS_OPERATOR_ROLE && responsibleSupplierId !== actor.supplierId) {
    throw codedError("只能录入本供应商负责的国内物流信息。", 403, "PERMISSION_DENIED");
  }
  const remarkText = domesticLogisticsRemark({ ...input, transportType });
  const finalFinanceStatus = actor.role === "管理员" && (input.financeStatus || input.status) === "CONFIRMED" ? "CONFIRMED" : "PENDING";
  const data = {
    orderId: order.id,
    transportType,
    ...
    remarkText,
    responsibleSupplierId: responsibleSupplierId || null,
    temporarySupplierName: temporarySupplierName || null,
    submittedByUserId: actor.id,
    submittedAt: new Date(),
    submitterRole: domesticLogisticsSubmitterRole(actor),
    financeStatus: finalFinanceStatus,
    financeConfirmedById: finalFinanceStatus === "CONFIRMED" ? actor.id : null,
    financeConfirmedAt: finalFinanceStatus === "CONFIRMED" ? new Date() : null,
    rejectReason: null,
...
export async function reviewDomesticLogisticsInfo(request, actor, id, input = {}) {
  assertWrite(actor, "taxRefund");
  const status = DOMESTIC_LOGISTICS_FINANCE_STATUSES.includes(input.financeStatus || input.status)
    ? (input.financeStatus || input.status)
    : "";
  if (!["CONFIRMED", "REJECTED"].includes(status)) throw codedError("请选择确认或驳回。", 400, "INVALID_FINANCE_STATUS");
  ...
  if (before.financeStatus !== "PENDING") throw codedError("只有待财务审核的国内物流信息可以确认或驳回。", 400, "INVALID_FINANCE_STATUS");
  if (status === "REJECTED" && !nonEmpty(input.rejectReason)) throw codedError("驳回需填写原因。", 400, "REJECT_REASON_REQUIRED");
  const row = await prisma.domesticLogisticsInfo.update({
    ...
    data: {
      ...
      rejectReason: status === "REJECTED" ? requireText(input.rejectReason, "驳回原因") : null,
      correctionRequested: false,
      correctionReason: null,
    },
    include: domesticLogisticsInclude(),
  });
  await runNonCriticalTask("国内物流审核操作日志写入", () => writeAudit(request, actor, status === "CONFIRMED" ? "确认国内物流信息" : "驳回国内物流信息", "domestic_logistics_infos", row.id, before, row));
  await runNonCriticalTask("退税资料完整度刷新", () => refreshTaxRefundCompleteness(row.orderId));
  return serializeDomesticLogisticsInfo(row);
}

export async function unlockDomesticLogisticsInfo(request, actor, id, input = {}) {
  if (actor?.role !== "管理员") throw codedError("只有管理员可以解锁已确认国内物流信息。", 403, "PERMISSION_DENIED");
  const before = await prisma.domesticLogisticsInfo.findFirst({
    where: { id, deletedAt: null },
    include: domesticLogisticsInclude(),
  });
  if (!before) throw codedError("国内物流信息不存在", 404, "DOMESTIC_LOGISTICS_NOT_FOUND");
  if (before.financeStatus !== "CONFIRMED") throw codedError("只有已确认记录需要解锁。", 400, "INVALID_FINANCE_STATUS");
  const row = await prisma.domesticLogisticsInfo.update({
    where: { id },
    data: {
      financeStatus: "PENDING",
      unlockedByUserId: actor.id,
      unlockedAt: new Date(),
      rejectReason: nonEmpty(input.reason || input.unlockReason) || null,
    },
    include: domesticLogisticsInclude(),
  });
  await runNonCriticalTask("国内物流解锁操作日志写入", () => writeAudit(request, actor, "解锁国内物流信息", "domestic_logistics_infos", row.id, before, row));
  await runNonCriticalTask("退税资料完整度刷新", () => refreshTaxRefundCompleteness(row.orderId));
  return serializeDomesticLogisticsInfo(row);
}
...
export async function requestDomesticLogisticsCorrection(request, actor, id, input = {}) {
  ...
  if (actor.role === LOGISTICS_OPERATOR_ROLE) {
    const supplierId = actorDomesticLogisticsSupplierId(actor);
    if (!supplierId || before.responsibleSupplierId !== supplierId) throw codedError("无权限申请更正该物流信息。", 403, "PERMISSION_DENIED");
  } else if (!canWrite(actor, "taxRefund")) {
...
export async function listOrderDocuments(query, actor) {
  ...
  const where = {
    ...
    ...(scope === "ALL" ? {} : { order: { is: orderAccessWhere(actor) } }),
...
async function resolveDocumentScope({ orderId, documentType, costId, supplierId }, actor) {
  ...
  if (!["ALL", "OWN"].includes(scope)) throw permissionError("无权限上传出口资料或销售合同");
