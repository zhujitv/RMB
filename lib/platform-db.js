...
export function hashPassword(password) {
  ...
  return scryptPasswordHash(plain);
}

export function upgradePasswordHash(password) {
  return scryptPasswordHash(password);
}
...
export function verifyPassword(password, passwordHash) {
  ...
  if (prefix !== PASSWORD_HASH_PREFIX || !salt || !derived) return false;
  try {
...
export function apiError(error, fallback = "请求处理失败") {
  ...
  return NextResponse.json(
    {
      error: isProduction && status >= 500 ? fallback : (error?.message || fallback),
...
function scopeOrderForActor(order, actor) {
  if (actor?.role !== "成本录入员" || !order) return order;
  return {
    ...order,
    payments: [],
...
function costAccessWhere(actor) {
  ...
  if (actor?.role === "成本录入员") return { createdById: actor.id };
  return {};
}
...
export async function searchReceivableOrders(query, actor) {
  ...
  const costEntrySearch = actor?.role === "成本录入员" && canCreateCostForOrder(actor);
  const filters = [
    costEntrySearch ? {} : orderAccessWhere(actor),
    q ? {
      OR: [
  ...
  const orders = await prisma.receivableOrder.findMany({
    where,
    include: costEntrySearch ? { customer: true, salesperson: true } : includeOrderRelations(),
    orderBy: [{ createdAt: "desc" }],
    take: 20,
  });
  return costEntrySearch ? orders.map(serializeReceivableSearchOrder) : orders.map(serializeOrder);
}
...
export async function uploadOrderDocument(request, actor, { orderId, documentType, file, costId = "", supplierId = "" }) {
  ...
  const storageKey = buildOrderDocumentKey({
    orderId: order.id,
    documentType,
    fileName: `${Date.now()}-${crypto.randomUUID()}-${originalFileName}`,
    relatedModule,
