...
import { buildOrderDocumentKey, readR2Object, r2BucketName, safeFileName, signedDownloadUrl, uploadToR2 } from "./r2";
...
export const ORDER_DOCUMENT_TYPES = Object.keys(ORDER_DOCUMENT_LABELS);
export const ORDER_DOCUMENT_ARCHIVE_NAMES = {
  CUSTOMS_ENTRY_FORM: "01_报关录入单",
  RELEASE_NOTICE: "02_放行通知书",
  CUSTOMS_POWER_OF_ATTORNEY: "03_报关委托书",
  BILL_OF_LADING: "04_提单",
  COMMERCIAL_INVOICE: "05_发票",
  PACKING_LIST: "06_箱单",
};
...
export async function getActor(request, { required = true } = {}) {
...
export function rolePermissions(user) {
  ...
  return {
    ...
    writes: Object.fromEntries(Object.keys(WRITE_PERMISSIONS).map((area) => [area, canWrite(user, area)])),
    reads: Object.fromEntries(Object.keys(READ_PERMISSIONS).map((area) => [area, canRead(user, area)])),
  };
}

export function canRead(user, area) {
  return READ_PERMISSIONS[area]?.includes(user?.role) || false;
}
...
export function documentCompleteness(documents = []) {
  const successTypes = new Set(
    documents
      .filter((document) => !document.deletedAt && document.uploadStatus === "SUCCESS")
      .map((document) => document.documentType),
  );
  const missingTypes = ORDER_DOCUMENT_TYPES.filter((type) => !successTypes.has(type));
  return {
    complete: missingTypes.length === 0,
    total: ORDER_DOCUMENT_TYPES.length,
    completed: ORDER_DOCUMENT_TYPES.length - missingTypes.length,
    missingTypes,
    missingLabels: missingTypes.map((type) => ORDER_DOCUMENT_LABELS[type] || type),
    text: missingTypes.length === 0 ? "单证完整" : `缺失：${missingTypes.map((type) => ORDER_DOCUMENT_LABELS[type] || type).join("、")}`,
  };
}
...
function derivedTaxRefundStatus(order, documents = order?.documents || []) {
  ...
  return documentCompleteness(documents).complete ? "READY" : "NOT_READY";
}
...
export function serializeOrder(order) {
  ...
  const completeness = documentCompleteness(order.documents || []);
...
function canAccessOrder(actor, order) {
  return canRead(actor, "orders") && (actor?.role !== "业务员" || order.createdById === actor.id || order.salespersonUserId === actor.id);
}
...
export async function saveCost(request, actor, input, id = null) {
  ...
  if (before && !canAccessOrder(actor, before.order)) throw permissionError("无权限修改该成本记录");
...
export async function deleteCost(request, actor, id) {
  ...
  if (!canAccessOrder(actor, before.order)) throw permissionError("无权限删除该成本记录");
...
async function assertDocumentOrder(orderId, actor) {
  ...
  const order = await prisma.receivableOrder.findFirst({
    where: { id: orderId, deletedAt: null },
    include: { customer: true, createdBy: true, salesperson: true },
  });
...
export async function listOrderDocuments(query, actor) {
  ...
  const rows = await prisma.orderDocument.findMany({
    where,
    include: { order: { include: { customer: true } }, uploadedBy: true },
    orderBy: [{ documentType: "asc" }, { createdAt: "desc" }],
  });
  return rows.map(serializeOrderDocument);
}

export async function uploadOrderDocument(request, actor, { orderId, documentType, file }) {
  ...
  if (!originalFileName.toLowerCase().endsWith(".pdf") || mimeType !== "application/pdf") {
    throw permissionError("只能上传 PDF 文件", 400);
  }
  const arrayBuffer = await file.arrayBuffer();
  const body = Buffer.from(arrayBuffer);
  const r2Key = buildOrderDocumentKey(order.id, documentType, originalFileName);
  const r2Bucket = r2BucketName();
  const pending = await prisma.orderDocument.create({
    data: {
      orderId: order.id,
      documentType,
  ...
  try {
    await uploadToR2({ key: r2Key, body, contentType: mimeType });
    const document = await prisma.orderDocument.update({
      where: { id: pending.id },
      data: { uploadStatus: "SUCCESS", uploadProgress: 100, uploadedAt: new Date() },
      include: { order: { include: { customer: true } }, uploadedBy: true },
    });
    ...
    await prisma.orderDocument.update({
      where: { id: pending.id },
      data: { uploadStatus: "FAILED", uploadProgress: 0 },
    }).catch(() => {});
    throw error;
  }
}
...
export async function deleteOrderDocument(request, actor, id) {
  ...
  if (!canAccessOrder(actor, before.order)) throw permissionError("无权限删除该订单单证");
  const document = await prisma.orderDocument.update({
    where: { id },
...
export async function getOrderDocumentDownload(request, actor, id) {
  ...
  if (!canAccessOrder(actor, document.order)) throw permissionError("无权限下载该订单单证");
  if (document.uploadStatus !== "SUCCESS") throw permissionError("文件尚未上传成功，不能下载", 400);
  const url = await signedDownloadUrl(document.r2Key, document.fileName);
...
export async function updateTaxRefundStatus(request, actor, orderId, status) {
  ...
  const completeness = documentCompleteness(before.documents || []);
...
function archiveFileName(type, index, total) {
  const base = ORDER_DOCUMENT_ARCHIVE_NAMES[type] || ORDER_DOCUMENT_LABELS[type] || type;
...
export async function buildTaxRefundPackage(request, actor, orderId, documentType = "") {
  ...
  const order = await prisma.receivableOrder.findFirst({
    where: { id: orderId, deletedAt: null },
    include: { customer: true, documents: { where: { deletedAt: null, uploadStatus: "SUCCESS" }, include: { uploadedBy: true } } },
  });
  if (!order) throw permissionError("应收订单不存在或已删除", 404);
  if (!canAccessOrder(actor, order)) throw permissionError("无权限下载该订单资料");
  const selectedTypes = ORDER_DOCUMENT_TYPES.includes(documentType) ? [documentType] : ORDER_DOCUMENT_TYPES;
  const documents = order.documents
    .filter((document) => selectedTypes.includes(document.documentType))
  ...
  for (const type of selectedTypes) {
    ...
    for (let index = 0; index < typeDocs.length; index += 1) {
      ...
      zip.file(archiveFileName(type, index, typeDocs.length), await readR2Object(document.r2Key));
    }
  }
