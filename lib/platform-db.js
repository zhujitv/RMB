...
async function applyCustomsParseFailure(request, actor, orderId, message, code = "CUSTOMS_PARSE_FAILED", action = "自动识别失败") {
  const before = await prisma.receivableOrder.findUnique({ where: { id: orderId } });
  if (!before || before.customsParseStatus === "MANUAL") return null;
...
export async function previewCustomsRecognition(actor, orderId) {
  assertCanManageCustomsRecognition(actor);
  if (actor?.role === LOGISTICS_OPERATOR_ROLE) throw permissionError("物流资料录入员不能重新识别报关单信息");
  const order = await prisma.receivableOrder.findFirst({
    where: { id: orderId, deletedAt: null, ...orderAccessWhere(actor) },
