...
const DEFAULT_INITIAL_PASSWORD = /^\d{8}$/.test(String(process.env.DEFAULT_INITIAL_PASSWORD || ""))
  ? String(process.env.DEFAULT_INITIAL_PASSWORD)
  : "12345678";
const INITIAL_ADMIN_PASSWORD = String(process.env.INITIAL_ADMIN_PASSWORD || DEFAULT_INITIAL_PASSWORD);
...
export async function saveUser(request, actor, input, id = null) {
  ...
  if (!id && !data.passwordHash) {
    data.passwordHash = hashPassword(DEFAULT_INITIAL_PASSWORD);
    data.mustChangePassword = true;
  }
