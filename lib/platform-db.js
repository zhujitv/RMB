...
function serializePayment(payment) {
  return {
    ...
    customerName: normalizeCustomerName(payment.order?.customerNameSnapshot || payment.order?.customer?.name || ""),
...
function serializeCost(cost) {
  return {
    ...
    customerName: normalizeCustomerName(cost.order?.customerNameSnapshot || cost.order?.customer?.name || ""),
...
function serializeOrderDocument(document) {
  return {
    ...
    customerName: normalizeCustomerName(document.order?.customerNameSnapshot || document.order?.customer?.name || ""),
...
export function serializeOrder(order) {
  ...
  return {
    ...
    customerName: normalizeCustomerName(order.customerNameSnapshot || order.customer?.name || ""),
    customerNameSnapshot: normalizeCustomerName(order.customerNameSnapshot || order.customer?.name || ""),
...
function isExternalLogisticsSupplierAccount(actor) {
  return actor?.role === LOGISTICS_OPERATOR_ROLE && Boolean(actor.supplierId);
}
...
async function buildCostData(order, actor, input, id = null, before = null) {
  ...
  const canConfirmOrdinaryCost = actor?.role === "管理员";
  ...
  if (requestedCostConfirmed && !canConfirmOrdinaryCost) {
    throw codedError("成本录入员只能录入成本，不能自行确认成本。", 403, "COST_CONFIRMATION_REQUIRES_REVIEWER");
  }
...
function serializeDomesticLogisticsOrder(order) {
  ...
  return {
    ...
    blNo: order.blNo || "",
    billOfLadingNo: order.blNo || "",
    customerShortName: normalizeCustomerName(order.customerNameSnapshot || order.customer?.name || ""),
    customerName: normalizeCustomerName(order.customerNameSnapshot || order.customer?.name || ""),
...
export async function listTaxRefundOrders(query, actor) {
  ...
  const declarationMonth = nonEmpty(query.get("declarationMonth"));
  const exportMonth = nonEmpty(query.get("exportMonth"));
  const declarationMonthStart = declarationMonth && /^\d{4}-\d{2}$/.test(declarationMonth) ? new Date(`${declarationMonth}-01T00:00:00.000Z`) : null;
  const declarationMonthEnd = declarationMonthStart ? new Date(Date.UTC(declarationMonthStart.getUTCFullYear(), declarationMonthStart.getUTCMonth() + 1, 1)) : null;
  const exportMonthStart = exportMonth && /^\d{4}-\d{2}$/.test(exportMonth) ? new Date(`${exportMonth}-01T00:00:00.000Z`) : null;
  const exportMonthEnd = exportMonthStart ? new Date(Date.UTC(exportMonthStart.getUTCFullYear(), exportMonthStart.getUTCMonth() + 1, 1)) : null;
  ...
  const where = {
    ...
    AND: [
      ...
      ...(monthStart && monthEnd ? [{
        ...(mode === "archive"
          ? { taxRefundArchivedAt: { gte: monthStart, lt: monthEnd } }
          : { updatedAt: { gte: monthStart, lt: monthEnd } }),
      }] : []),
      ...(declarationMonthStart && declarationMonthEnd ? [{ customsDeclarationDate: { gte: declarationMonthStart, lt: declarationMonthEnd } }] : []),
      ...(exportMonthStart && exportMonthEnd ? [{ customsExportDate: { gte: exportMonthStart, lt: exportMonthEnd } }] : []),
    ],
  };
  const [total, rows] = await Promise.all([
    prisma.receivableOrder.count({ where }),
