...
const LEGACY_LOGISTICS_OPERATOR_ROLE = "物流资料录入员";
export const ROLES = ["管理员", "业务员", "财务", "成本录入员", LOGISTICS_OPERATOR_ROLE, LEGACY_LOGISTICS_OPERATOR_ROLE, "查看者"];
...
const LOGISTICS_INVOICE_ENGLISH_LABELS = {
...
const CUSTOMS_DECLARATION_DOCUMENT_TYPE_ALIASES = new Set(["CUSTOMS_ENTRY_FORM", "CUSTOMS_DECLARATION", "报关单"]);
...
const CUSTOMS_FILE_READ_FAILED_MESSAGE = "报关单文件读取失败，请重新上传或联系管理员。";
const SHIPPING_DOCUMENT_TYPE_KEYS = ["commercialInvoice", "packingList", "customsDeclaration"];
const DEFAULT_SHIPPING_DOCUMENT_TYPES = [...SHIPPING_DOCUMENT_TYPE_KEYS];
const SHIPPING_DOCUMENT_TYPE_CONFIG = {
...
const SHIPPING_NOTIFICATION_STATUS_LABELS = {
  NOT_ENABLED: "未启用",
...
const SHIPPING_EMAIL_LANGUAGE_LABELS = {
  EN: "English",
  RU: "Русский",
  en: "English",
  ru: "Русский",
...
function documentStandardTypeKey(document = {}) {
...
function orderDocumentsForStandardNaming(order = {}) {
  return (order.documents || [])
    .filter((document) => document && !document.deletedAt && document.uploadStatus === "SUCCESS")
    .slice()
    .sort((a, b) => (
      new Date(a.uploadedAt || a.createdAt || 0) - new Date(b.uploadedAt || b.createdAt || 0)
      || String(a.id || "").localeCompare(String(b.id || ""))
    ));
}

function standardFilenameIndexForDocument(order = {}, document = {}) {
  const targetType = documentStandardTypeKey(document);
  const docs = orderDocumentsForStandardNaming(order).filter((item) => documentStandardTypeKey(item) === targetType);
  const index = docs.findIndex((item) => item.id === document.id);
  return index >= 0 ? index + 1 : docs.length + 1;
}

function standardFilenameForDocument(document = {}, orderOverride = null) {
  if (document.standardFilename) return document.standardFilename;
...
async function nextStandardFilenameForUpload(order = {}, documentType = "", context = {}) {
  const standardType = englishDocumentTypeLabel(documentType, context);
  const documents = await prisma.orderDocument.findMany({
    where: { orderId: order.id, deletedAt: null, uploadStatus: "SUCCESS" },
    include: { cost: true },
  });
  const nextIndex = documents.filter((document) => documentStandardTypeKey(document) === standardType).length + 1;
  return generateStandardFilename(order, documentType, nextIndex, context);
}

async function resolveStandardFilenameForPersistedDocument(document = {}) {
  if (document.standardFilename) return document.standardFilename;
  if (!document.orderId) return standardFilenameForDocument(document);
  ...
  const hydrated = (order?.documents || []).find((item) => item.id === document.id) || document;
  return standardFilenameForDocument({ ...document, ...hydrated }, order || document.order || {});
}

function normalizeUploadSource(uploadSource = "", relatedModule = "") {
  const value = String(uploadSource || "").trim().toUpperCase();
  const allowed = new Set(["TAX_REFUND_DETAIL", "DOMESTIC_LOGISTICS", "LOGISTICS_SUPPLIER_PORTAL", "ORDER_DOCUMENTS"]);
  if (allowed.has(value)) return value;
  return relatedModule === "SUPPLIER" ? "LOGISTICS_SUPPLIER_PORTAL" : "ORDER_DOCUMENTS";
}

function normalizeShippingDocumentTypes(value) {
  const source = Array.isArray(value) ? value : DEFAULT_SHIPPING_DOCUMENT_TYPES;
  const types = source.filter((item) => SHIPPING_DOCUMENT_TYPE_KEYS.includes(item));
  return types.length ? types.filter((item, index, arr) => arr.indexOf(item) === index) : DEFAULT_SHIPPING_DOCUMENT_TYPES;
}
...
const WRITE_PERMISSIONS = {
  ...
  payments: ["管理员", "财务"],
  costs: ["管理员", "业务员", "成本录入员"],
  logistics: ["管理员", LOGISTICS_OPERATOR_ROLE],
  domesticLogistics: ["管理员", "业务员", LOGISTICS_OPERATOR_ROLE, LEGACY_LOGISTICS_OPERATOR_ROLE],
  documents: ["管理员", "业务员", "财务", "成本录入员", LOGISTICS_OPERATOR_ROLE, LEGACY_LOGISTICS_OPERATOR_ROLE],
  taxRefund: ["管理员", "财务"],
  commissions: ["管理员", "财务"],
  suppliers: ["管理员"],
  settings: ["管理员"],
...
export const ROLE_MENUS = {
  ...
  成本录入员: ["costs", "manual"],
  [LOGISTICS_OPERATOR_ROLE]: ["domesticLogistics", "manual"],
  [LEGACY_LOGISTICS_OPERATOR_ROLE]: ["domesticLogistics", "manual"],
  查看者: ["manual"],
};
...
export const ROLE_SCOPE_TEXT = {
  ...
  [LOGISTICS_OPERATOR_ROLE]: "仅可查看分配订单、提交物流费用并上传发票",
  [LEGACY_LOGISTICS_OPERATOR_ROLE]: "可录入国内物流信息和报关资料",
  查看者: "只读权限",
};
...
function roleDataScope(role) {
  ...
  if (role === LEGACY_LOGISTICS_OPERATOR_ROLE) return "OWN";
  return "NONE";
}
...
export function getCurrentUserScope(user) {
...
export function requirePermission(user, area, action = "read") {
  if (action === "write") return assertWrite(user, area);
  return assertRead(user, area);
}

export function requireDataScope(user) {
...
export function requireAdminGlobal(user, message = "无权限访问全局数据") {
...
function requireValidEmailList(value, label) {
...
function normalizeLogisticsCostTypeList(value = []) {
...
function serializeOrderDocument(document, orderOverride = null) {
...
function shippingNotificationStatus(row = null, customer = null) {
...
function serializeShippingDocumentNotification(row = null, order = {}) {
...
function serializeDomesticLogisticsTransportItem(item) {
  if (!item) return null;
  return {
    id: item.id,
...
export async function listSuppliers(query, actor = null, onlyActive = false, options = {}) {
  ...
  const typeMap = {
    factory: "工厂供应商",
    logistics: "物流供应商",
    logisticsfee: ["物流供应商", "海运供应商"],
    "logistics-fee": ["物流供应商", "海运供应商"],
    logistics_fee: ["物流供应商", "海运供应商"],
    customs: "报关供应商",
    ocean: "海运供应商",
    shipping: "海运供应商",
    other: "其他供应商",
...
async function defaultOrderLogisticsSupplier() {
...
function normalizeIdList(value = []) {
  return normalizedStringArray(value).filter((item, index, arr) => item && arr.indexOf(item) === index);
}

async function syncOrderLogisticsSuppliers(orderId, supplierIds = [], actor = null) {
  const settings = await getExchangeRateSettings();
  let ids = normalizeIdList(supplierIds);
...
function documentOrderListAccessWhere(actor, documentType) {
  const scope = effectivePermissions(actor).dataScope;
  if (scope === "ALL") return {};
...
function includeLogisticsExpenseRelations() {
  return {
    order: {
      include: {
        customer: true,
...
function logisticsExpenseOrderSummary(order = {}) {
...
function serializeLogisticsExpense(expense = {}) {
...
function logisticsExpenseAccessWhere(actor, { forList = true } = {}) {
...
function assertCanReadLogisticsExpenses(actor) {
  if (["管理员", "财务", "业务员", LOGISTICS_OPERATOR_ROLE, LEGACY_LOGISTICS_OPERATOR_ROLE].includes(actor?.role)) return;
  throw permissionError("无权限查看物流费用", 403);
}

function assertCanWriteLogisticsExpense(actor) {
  if (["管理员", LOGISTICS_OPERATOR_ROLE, LEGACY_LOGISTICS_OPERATOR_ROLE].includes(actor?.role)) return;
  throw permissionError("无权限录入物流费用", 403);
}

function assertCanReviewLogisticsExpense(actor) {
  if (actor?.role === "管理员") return;
  throw permissionError("只有管理员可以审核物流费用", 403);
}

function assertCanConfirmLogisticsInvoice(actor) {
  if (["管理员", "财务"].includes(actor?.role)) return;
  throw permissionError("只有管理员或财务可以确认物流发票", 403);
}

function logisticsExpenseStatusWhere(status = "") {
...
export async function listLogisticsExpenses(query, actor) {
  ...
  const costType = normalizedCostType(nonEmpty(query.get("costType")));
  const where = {
    deletedAt: null,
...
async function assertLogisticsExpenseOrder(input = {}, actor) {
...
async function assertLogisticsExpenseSupplier(actor, order, input = {}) {
...
async function buildLogisticsExpenseData(order, supplier, actor, input = {}, before = null) {
  const inputCostType = normalizedCostType(input.costType);
...
async function loadLogisticsExpenseForAction(id, actor) {
  const expense = await prisma.logisticsExpense.findFirst({
    where: {
      id,
      deletedAt: null,
      ...logisticsExpenseAccessWhere(actor, { forList: false }),
    },
    include: includeLogisticsExpenseRelations(),
  });
...
async function createOrUpdateCostFromLogisticsExpense(tx, expense, actor) {
  ...
  if (existing) {
    return tx.orderCost.update({ where: { id: existing.id }, data: costData });
  }
...
async function notifyLogisticsSupplierInvoice(expense) {
...
function assertInvoiceFileSignature(body, mimeType) {
  if (mimeType === "application/pdf") {
    if (body.byteLength < 5 || body.subarray(0, 5).toString("ascii") !== "%PDF-") throw codedError("文件格式错误，只能上传有效 PDF 或图片文件。", 400, "FILE_SIGNATURE_INVALID");
  ...
  if (mimeType === "image/png" && hex.startsWith("89504e470d0a1a0a")) return;
  if (mimeType === "image/jpeg" && hex.startsWith("ffd8ff")) return;
  if (mimeType === "image/webp" && ascii.startsWith("RIFF") && ascii.slice(8, 12) === "WEBP") return;
  throw codedError("文件格式错误，只能上传有效 PDF、JPG、PNG 或 WEBP 文件。", 400, "FILE_SIGNATURE_INVALID");
}

async function createLogisticsInvoiceDocument(request, actor, expense, file, metadata = {}) {
  ...
  if (!extension) throw codedError("文件类型不允许，只能上传 PDF、JPG、PNG 或 WEBP 文件。", 400, "FILE_TYPE_NOT_ALLOWED");
...
function canUploadLogisticsExpenseInvoice(actor, expense) {
...
export async function uploadLogisticsExpenseInvoice(request, actor, id, formData) {
  ...
  if (!file || typeof file.arrayBuffer !== "function") throw codedError("请上传发票 PDF 或图片。", 400, "INVOICE_FILE_REQUIRED");
...
function normalizeDomesticTransportItems(input = {}, transportType = "TRUCK") {
...
function sortDomesticLogisticsOrders(a = {}, b = {}) {
...
export async function listDomesticLogisticsOrders(query, actor) {
  ...
  } else if (actor?.role === LOGISTICS_OPERATOR_ROLE) {
  ...
  )).sort(sortDomesticLogisticsOrders).map(serializeDomesticLogisticsOrder);
}
...
function canReadProtectedCustomsDocumentContent(actor) {
  return ["管理员", "财务"].includes(actor?.role) || isInternalLogisticsOperator(actor);
}

function canReadDocumentContent(actor, document) {
  if (!canReadDocument(actor, document)) return false;
  if (isProtectedCustomsDocumentType(document.documentType)) {
    return canReadProtectedCustomsDocumentContent(actor);
  }
  return true;
}
...
function shippingInvoiceNo(order = {}, commercialInvoiceDocument = null) {
  const name = commercialInvoiceDocument
    ? standardFilenameForDocument(commercialInvoiceDocument, order)
    : "";
  const base = name.replace(/\.pdf$/i, "").trim();
  return base || order.orderNo || "-";
}
...
async function sendShippingDocumentsEmail({ recipientEmails, ccEmails, attachments, subject, body, notificationId }) {
...
async function tryAutoShippingDocumentsNotification(request, actor, orderId) {
  try {
...
export async function updateCustomsRecognition(request, actor, orderId, input = {}) {
  ...
  return notifiedOrder || serializeOrder(await loadOrderForShippingNotification(order.id, actor));
}
...
function sortTaxRefundOrders(a = {}, b = {}) {
...
export async function listTaxRefundOrders(query, actor) {
  ...
  const hydratedRows = rows.map((order) => refreshedById[order.id]
    ...
    : order).sort(sortTaxRefundOrders);
...
function supplierArchiveFileName(document, index, total, order = {}) {
