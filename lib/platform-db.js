...
export const ORDER_DOCUMENT_LABELS = {
  ...
  PACKING_LIST: "箱单",
  SALES_CONTRACT: "销售合同",
  SUPPLIER_PURCHASE_CONTRACT: "采购合同",
...
export function taxDocumentCompleteness(order = {}) {
  ...
  const costs = (order.costs || []).filter((cost) => !cost.deletedAt && cost.supplierId);
  ...
  const exportMissing = EXPORT_DOCUMENT_TYPES.filter((type) => !hasOrderType(type));
  const salesMissing = SALES_DOCUMENT_TYPES.filter((type) => !hasOrderType(type));
  ...
  supplierEntries.forEach((entry) => {
    ...
    SUPPLIER_DOCUMENT_TYPES.forEach((type) => {
      ...
      if (!exists) supplierMissing.push({
        ...
        documentType: type,
        label: `${entry.supplierName}${type === "SUPPLIER_PURCHASE_CONTRACT" ? "采购合同" : "进项发票"}`,
        reminderDue: daysSinceCostCreated >= 3,
        daysSinceCostCreated,
      });
    });
  });
  const exportCompleted = EXPORT_DOCUMENT_TYPES.length - exportMissing.length;
  const salesCompleted = SALES_DOCUMENT_TYPES.length - salesMissing.length;
  ...
  const missingLabels = [
    ...
    ...salesMissing.map((type) => ORDER_DOCUMENT_LABELS[type] || type),
    ...supplierMissing.map((item) => item.label),
  ];
  const total = EXPORT_DOCUMENT_TYPES.length + SALES_DOCUMENT_TYPES.length + supplierTotal;
  const completed = exportCompleted + salesCompleted + supplierCompleted;
  return {
    complete: missingLabels.length === 0,
    total,
    completed,
    missingTypes: [...exportMissing, ...salesMissing, ...supplierMissing.map((item) => item.documentType)],
    missingLabels,
    export: { completed: exportCompleted, total: EXPORT_DOCUMENT_TYPES.length, missingTypes: exportMissing },
    sales: { completed: salesCompleted, total: SALES_DOCUMENT_TYPES.length, missingTypes: salesMissing },
    supplier: {
...
async function resolveDocumentScope({ orderId, documentType, costId, supplierId }, actor) {
  ...
  if (!["管理员", "业务员"].includes(actor.role)) throw permissionError("无权限上传出口或销售资料");
...
export async function uploadOrderDocument(request, actor, { orderId, documentType, file, costId = "", supplierId = "" }) {
  ...
  const r2Key = buildOrderDocumentKey({
    orderId: order.id,
    documentType,
...
function archiveFileName(type, index, total, supplierName = "") {
  ...
  if (supplierName) return `03_供应商资料/${safeFileName(supplierName)}/${base}${total > 1 ? `_${index + 1}` : ""}.pdf`;
    ...
    if (SUPPLIER_DOCUMENT_TYPES.includes(type)) {
      ...
      for (const group of groups) {
        for (let index = 0; index < group.length; index += 1) {
          ...
          const supplierName = document.supplier?.supplierName || document.cost?.supplierNameSnapshot || document.cost?.supplier?.supplierName || "未命名供应商";
          zip.file(archiveFileName(type, index, group.length, supplierName), await readR2Object(document.r2Key));
        }
      }
    } else {
      const folder = SALES_DOCUMENT_TYPES.includes(type) ? "02_销售资料" : "01_出口资料";
      for (let index = 0; index < typeDocs.length; index += 1) {
        const document = typeDocs[index];
        zip.file(`${folder}/${archiveFileName(type, index, typeDocs.length)}`, await readR2Object(document.r2Key));
      }
    }
  }
