...
const ACTIVE_TAX_REFUND_STATUSES = ["NOT_READY", "READY", "SUBMITTED", "PROBLEM"];
const ARCHIVE_TAX_REFUND_STATUSES = ["COMPLETED", "ARCHIVED"];
...
function derivedTaxRefundStatus(order, documents = order?.documents || []) {
  ...
  if (["SUBMITTED", "COMPLETED", "ARCHIVED", "PROBLEM"].includes(status)) return status;
...
function taxRefundStatusFromCompleteness(currentStatus, completeness) {
  if (["SUBMITTED", "COMPLETED", "ARCHIVED", "PROBLEM"].includes(currentStatus)) return currentStatus;
...
function canModifyDocument(actor, document) {
  ...
  if (document.order?.taxRefundStatus === "ARCHIVED") return false;
...
async function resolveDocumentScope({ orderId, documentType, costId, supplierId }, actor) {
  ...
  if (order.taxRefundStatus === "ARCHIVED") throw permissionError("已办结归档订单只允许查看和下载资料");
  const scope = effectivePermissions(actor).dataScope;
...
export async function listTaxRefundOrders(query, actor) {
  ...
  const where = {
    ...
    AND: [
      orderAccessWhere(actor),
      keywordWhere,
      { taxRefundStatus: { in: mode === "archive" ? ARCHIVE_TAX_REFUND_STATUSES : ACTIVE_TAX_REFUND_STATUSES } },
...
export async function updateTaxRefundStatus(request, actor, orderId, status, input = {}) {
  ...
  if (before.taxRefundStatus === "ARCHIVED" && status !== "ARCHIVED") {
    throw permissionError("已办结归档订单不能继续修改退税状态。", 400);
  }
  if (before.taxRefundStatus === "COMPLETED" && status !== "ARCHIVED" && status !== "COMPLETED") {
    throw permissionError("退税完成订单只能继续办结归档。", 400);
  }
  const completeness = taxDocumentCompleteness(before);
  if (status === "ARCHIVED" && before.taxRefundStatus !== "COMPLETED") {
    throw permissionError("只有退税完成的订单才能办结归档。", 400);
  }
  ...
  const order = await prisma.receivableOrder.update({
    ...
    data: {
      ...
      ...(status === "ARCHIVED" ? {
        ...
        taxRefundArchivedAt: new Date(),
        taxRefundArchiveRemark: archiveRemark,
      } : {}),
    },
    include: includeOrderRelations(),
  });
  await runNonCriticalTask("退税状态操作日志写入", () => writeAudit(request, actor, status === "ARCHIVED" ? "退税资料办结归档" : "修改退税状态", "receivable_orders", order.id, before, order));
