...
function canClaimDomesticLogisticsOrder(actor, order = {}, input = {}) {
  if (actor?.role !== LOGISTICS_OPERATOR_ROLE) return false;
...
export async function savePayment(request, actor, input, id = null) {
  ...
  const confirmStatuses = ["已到账", "部分到账"];
  if (!id && confirmStatuses.includes(requestedStatus)) {
    throw codedError("新增收款只能保存为待确认，请由另一位财务或管理员复核到账。", 400, "PAYMENT_CONFIRMATION_REQUIRED");
  }
  if (id && before && !confirmStatuses.includes(before.status) && confirmStatuses.includes(requestedStatus)) {
    if (before.createdById === actor.id && actor.role !== "管理员") {
      throw codedError("收款录入人不能确认自己录入的收款，请由另一位财务或管理员复核。", 403, "PAYMENT_CONFIRMATION_REQUIRES_REVIEWER");
    }
  }
  const data = {
    orderId: order.id,
...
export async function listDomesticLogisticsOrders(query, actor) {
  ...
  if (actor?.role === LOGISTICS_OPERATOR_ROLE && !keyword) {
  ...
  return orders.filter((order) => (
    actor?.role === LOGISTICS_OPERATOR_ROLE && keyword
      ? true
