...
export const PAYMENT_TERMS = ["30%预付款", "50%预付款", "100%预付款", "自定义比例", "预付款", "见提单付款", "见提单复印件付款", "OA账期", "到港后付款", "分批付款", "其他"];
...
function depositRatioForTerm(paymentTerm, inputRatio = null) {
  if (paymentTerm === "30%预付款") return 0.3;
  if (paymentTerm === "50%预付款") return 0.5;
  if (paymentTerm === "100%预付款") return 1;
  if (paymentTerm !== "自定义比例") return null;
  const ratio = num(inputRatio);
  if (ratio <= 0 || ratio > 100) {
    const error = new Error("自定义预付款比例必须大于 0 且不超过 100%");
    error.status = 400;
    throw error;
  }
...
export async function saveOrder(request, actor, input, id = null) {
  ...
  const paymentTerm = PAYMENT_TERMS.includes(input.paymentTerm) ? input.paymentTerm : "OA账期";
  const depositRatio = depositRatioForTerm(paymentTerm, input.depositRatio);
