...
export const DOMESTIC_LOGISTICS_FINANCE_STATUSES = ["PENDING", "CONFIRMED", "REJECTED"];
...
const WRITE_PERMISSIONS = {
  ...
  costs: ["管理员", "成本录入员"],
  logistics: ["管理员", "业务员"],
  domesticLogistics: ["管理员", "财务", LOGISTICS_OPERATOR_ROLE],
...
export async function listSuppliers(query, actor = null, onlyActive = false, options = {}) {
  ...
  } else if (!canRead(actor, "suppliers") && !canWrite(actor, "costs") && !canWrite(actor, "logistics")) {
...
export async function listDomesticLogisticsOrders(query, actor) {
  ...
  const where = {
    ...
    ...(supplierId ? {
      OR: [
        { domesticLogisticsInfos: { some: { responsibleSupplierId: supplierId } } },
...
export async function saveDomesticLogisticsInfo(request, actor, input, id = null) {
  ...
  const before = id
    ? await prisma.domesticLogisticsInfo.findFirst({ where: { id }, include: domesticLogisticsInclude() })
    : ((order.domesticLogisticsInfos || [])[0] || null);
  if (id && !before) throw codedError("国内物流信息不存在", 404, "DOMESTIC_LOGISTICS_NOT_FOUND");
  if (before?.financeStatus === "CONFIRMED" && actor.role === LOGISTICS_OPERATOR_ROLE && !before.correctionRequested) {
    throw codedError("财务已确认的物流信息已锁定，如需修改请先申请更正。", 400, "DOMESTIC_LOGISTICS_LOCKED");
  }
  const transportType = DOMESTIC_LOGISTICS_TRANSPORT_TYPES.includes(input.transportType) ? input.transportType : "TRUCK";
  const responsibleSupplierId = input.responsibleSupplierId
  ...
  if (!responsibleSupplierId) {
    throw codedError("存在多个或未识别物流供应商，请先由财务或管理员指定国内物流负责供应商。", 400, "RESPONSIBLE_SUPPLIER_REQUIRED");
  }
