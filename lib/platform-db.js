...
export function summarizeOrder(order) {
  ...
  const receivableAmount = Number(order.receivableAmount);
  ...
  const outstandingAmount = Math.max(receivableAmount - (confirmedPaymentsCny / exchangeRate), 0);
...
async function assertOrderCanReceivePayment(order) {
  ...
  const outstandingCny = Number(order.receivableAmountCny) - Number(confirmed._sum.amountCny || 0);
  if (outstandingCny <= 0) {
...
function buildCostData(order, actor, input, id = null) {
...
function includeCostRelations() {
  return { order: { include: { customer: true } }, createdBy: true, updatedBy: true };
}
...
export async function saveCost(request, actor, input, id = null) {
  ...
  const data = buildCostData(order, actor, input, id);
...
export async function saveCosts(request, actor, input) {
  ...
  const rows = items.map((item) => buildCostData(order, actor, {
    ...input,
    ...item,
