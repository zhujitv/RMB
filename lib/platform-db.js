...
export const COST_TYPES = [...FACTORY_SUPPLIER_COST_TYPES, "国内物流费", ...LOGISTICS_COST_TYPES, "佣金", "样品费", "银行手续费", "其他费用"]
  .filter((item, index, arr) => arr.indexOf(item) === index);
...
async function buildCostData(order, actor, input, id = null, before = null) {
  ...
  const currency = nonEmpty(input.currency).toUpperCase();
  if (!currency) throw codedError("请选择成本币种", 400, "CURRENCY_REQUIRED");
  if (!CURRENCIES.includes(currency)) {
    const error = new Error("请选择有效成本币种");
    error.status = 400;
    throw error;
  }
  const exchangeRateInput = currency === "CNY" && !nonEmpty(input.exchangeRate) ? 1 : input.exchangeRate;
  ...
  const exchange = await resolveExchangeRateSnapshot({ ...input, exchangeRate: exchangeRateInput }, actor, {
    currency,
...
export async function saveCost(request, actor, input, id = null) {
  ...
  await refreshTaxRefundCompleteness(cost.orderId);
...
export async function deleteCost(request, actor, id) {
  ...
  await refreshTaxRefundCompleteness(before.orderId);
}
