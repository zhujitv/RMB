...
export async function updateTaxRefundStatus(request, actor, orderId, status, input = {}) {
  ...
  if (["SUBMITTED", "COMPLETED", "ARCHIVED"].includes(before.taxRefundStatus) && status !== "SUBMITTED") {
  ...
  await runNonCriticalTask("退税状态操作日志写入", () => writeAudit(request, actor, status === "SUBMITTED" ? "提交退税并归档" : "修改退税状态", "receivable_orders", order.id, before, order));
