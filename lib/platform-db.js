...
export async function updateOwnProfile(request, actor, input = {}) {
  ...
  await writeAudit(request, actor, "修改本人资料", "users", user.id, before, publicUser(updated));
  return publicUser(updated);
}
...
export async function registerUser(request, input = {}) {
  ...
  await writeAudit(request, null, "用户自助注册", "users", user.id, null, {
    id: user.id,
    email: user.email,
    name: user.name,
...
export async function saveUser(request, actor, input, id = null) {
  ...
  const approvalStatus = USER_APPROVAL_STATUSES.includes(input.approvalStatus)
    ...
    : (input.isActive === false ? "DISABLED" : (before?.approvalStatus || "APPROVED"));
  const data = {
...
export async function getOrderDocumentPreview(request, actor, id) {
  ...
  await writeAudit(request, actor, "预览文件", "order_documents", document.id, null, {
    orderNo: document.order?.orderNo,
    fileName: document.fileName,
  });
