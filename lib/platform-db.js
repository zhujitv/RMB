...
const WRITE_PERMISSIONS = {
  ...
  domesticLogistics: ["管理员", "业务员"],
  documents: ["管理员", "业务员", "财务", "成本录入员"],
  taxRefund: ["管理员", "财务"],
  commissions: ["管理员", "财务"],
  suppliers: ["管理员"],
  settings: ["管理员"],
...
async function syncOrderLogisticsSuppliers(orderId, supplierIds = [], actor = null) {
  const ids = normalizeIdList(supplierIds);
  if (ids.length) {
    const suppliers = await prisma.supplier.findMany({
...
function canAccessDomesticLogisticsOrder(actor, order) {
  ...
  if (isExternalLogisticsSupplierAccount(actor)) {
    return (order.logisticsSuppliers || []).some((row) => row.supplierId === actor.supplierId)
      || (order.domesticLogisticsInfos || []).some((info) => !info.deletedAt && info.submittedByUserId === actor.id);
  }
...
function isInternalLogisticsOperator(actor) {
  return [LOGISTICS_OPERATOR_ROLE, LEGACY_LOGISTICS_OPERATOR_ROLE].includes(actor?.role) && !actor.supplierId;
}
...
function documentOrderListAccessWhere(actor, documentType) {
  ...
  if (canUseDomesticLogisticsDocumentScope(actor, documentType)) {
    ...
    if (isExternalLogisticsSupplierAccount(actor)) {
      return { order: { is: { domesticLogisticsInfos: { some: { submittedByUserId: actor.id, deletedAt: null } } } } };
    }
  }
...
function logisticsExpenseAccessWhere(actor, { forList = true } = {}) {
  ...
  if ([LOGISTICS_OPERATOR_ROLE, LEGACY_LOGISTICS_OPERATOR_ROLE].includes(actor?.role)) {
    ...
    return forList ? { createdById: actor.id } : { OR: [{ createdById: actor.id }, { supplierId: "__no_supplier__" }] };
  }
