...
export const LOGISTICS_SUPPLIER_ROLE = "物流供应商";
export const ROLES = ["管理员", "业务员", "财务", "成本录入员", LOGISTICS_OPERATOR_ROLE, LOGISTICS_SUPPLIER_ROLE, "查看者"];
...
export const LOGISTICS_SUPPLIER_EXPENSE_TYPES = [...LOGISTICS_COST_TYPES, "自定义费用"];
export const LOGISTICS_SUPPLIER_EXPENSE_REVIEW_STATUSES = ["草稿", "已提交", "已审核", "已退回", "已作废"];
export const LOGISTICS_SUPPLIER_EXPENSE_PAYMENT_STATUSES = ["未付款", "已申请付款", "已付款"];
export const LOGISTICS_SUPPLIER_EXPENSE_INVOICE_STATUSES = ["未上传", "已上传", "已审核", "已退回"];
export const LOGISTICS_SUPPLIER_EXPENSE_DOCUMENT_TYPES = ["发票", "对账单", "报关费发票", "拖车费发票", "港杂费发票", "其他证明资料"];
export const LOGISTICS_SUPPLIER_EXPENSE_TAX_DOCUMENT_TYPES = ["不计入退税资料", "报关费发票", "拖车费发票", "港杂费发票", "其他物流资料"];
...
const SEA_FREIGHT_REQUIREMENT_KEY = "SEA";
const SEA_FREIGHT_REQUIRED_TRADE_TERMS = ["CIF", "CFR"];
const TAX_REFUND_LOGISTICS_RULE_VERSION = "TRADE_TERM_SEA_20260607";
...
export const CUSTOMS_PARSE_STATUSES = ["SUCCESS", "FAILED", "MANUAL"];
export const CUSTOMS_PARSE_STATUS_LABELS = {
  SUCCESS: "成功",
  FAILED: "失败",
  MANUAL: "人工修改",
};
const ACTIVE_TAX_REFUND_STATUSES = ["NOT_READY", "READY", "PROBLEM"];
const ARCHIVE_TAX_REFUND_STATUSES = ["SUBMITTED", "COMPLETED", "ARCHIVED"];
...
const WRITE_PERMISSIONS = {
  ...
  costs: ["管理员", "业务员", "成本录入员"],
  logistics: ["管理员", "业务员"],
  logisticsExpenses: ["管理员", "财务", "成本录入员", LOGISTICS_SUPPLIER_ROLE],
...
export const ROLE_MENUS = {
  ...
  [LOGISTICS_OPERATOR_ROLE]: ["domesticLogistics", "logisticsExpenses"],
  [LOGISTICS_SUPPLIER_ROLE]: ["logisticsExpenses"],
...
export const ROLE_SCOPE_TEXT = {
  ...
  [LOGISTICS_SUPPLIER_ROLE]: "仅可登记本供应商物流费用",
  查看者: "只读权限",
};
...
const READ_PERMISSIONS = {
  ...
  costs: ["管理员", "财务", "成本录入员", "查看者"],
  logisticsExpenses: ["管理员", "业务员", "财务", "成本录入员", LOGISTICS_OPERATOR_ROLE, LOGISTICS_SUPPLIER_ROLE],
  domesticLogistics: ["管理员", "业务员", "财务", LOGISTICS_OPERATOR_ROLE, "查看者"],
  documents: ["管理员", "业务员", "财务", "成本录入员", LOGISTICS_OPERATOR_ROLE, LOGISTICS_SUPPLIER_ROLE, "查看者"],
  taxRefund: ["管理员", "财务"],
...
function roleDataScope(role) {
  ...
  if (role === LOGISTICS_SUPPLIER_ROLE) return "OWN";
  return "NONE";
}
...
function effectivePermissions(user) {
  ...
  if (role === LOGISTICS_OPERATOR_ROLE || role === LOGISTICS_SUPPLIER_ROLE) return base;
...
export function canRead(user, area) {
  if (user?.role === "管理员" && area === "logisticsExpenses") return true;
...
function customerFullName(customer, snapshot = "") {
...
function customerBusinessName(customer, snapshot = "") {
...
function serializeOrderDocument(document) {
  return {
    ...
    logisticsSupplierExpenseId: document.logisticsSupplierExpenseId || "",
    logisticsDocumentType: document.logisticsDocumentType || "",
...
function displayLogisticsSupplierExpenseName(expense = {}) {
  return nonEmpty(expense.customExpenseName) || expense.expenseType || "";
}

function serializeLogisticsSupplierExpense(expense) {
  const fullCustomerName = customerFullName(expense.order?.customer, expense.order?.customerNameSnapshot);
  const shortCustomerName = customerShortName(expense.order?.customer);
  return {
    id: expense.id,
    orderId: expense.orderId,
    orderNo: expense.order?.orderNo || "",
    blNo: expense.order?.blNo || "",
    billOfLadingNo: expense.order?.blNo || "",
    ...
    supplierType: expense.supplier?.supplierType || "",
    orderCostId: expense.orderCostId || expense.orderCost?.id || "",
    expenseType: expense.expenseType,
    customExpenseName: expense.customExpenseName || "",
    displayExpenseName: displayLogisticsSupplierExpenseName(expense),
    taxDocumentType: expense.taxDocumentType || "不计入退税资料",
    amount: Number(expense.amount || 0),
    currency: expense.currency || "CNY",
    exchangeRate: Number(expense.exchangeRate || 1),
    amountCny: Number(expense.amountCny || 0),
    expenseDate: dateToInput(expense.expenseDate),
    settlementMonth: expense.settlementMonth || "",
    invoiceStatus: expense.invoiceStatus || "未上传",
    paymentStatus: expense.paymentStatus || "未付款",
    reviewStatus: expense.reviewStatus || "草稿",
    ...
    documents: (expense.documents || []).map(serializeOrderDocument),
    createdAt: expense.createdAt,
    updatedAt: expense.updatedAt,
  };
}

function serializeCustomsRecognition(order = {}) {
  return {
    customsDeclarationNo: order.customsDeclarationNo || "",
    customsDeclarationDate: dateToInput(order.customsDeclarationDate),
    customsExportDate: dateToInput(order.customsExportDate),
    customsPort: order.customsPort || "",
    customsParsedAt: order.customsParsedAt || null,
    customsParseStatus: order.customsParseStatus || "",
    customsParseStatusLabel: customsParseStatusLabel(order.customsParseStatus),
    customsParseMessage: order.customsParseMessage || "",
  };
}
...
function customsParseStatusLabel(status) {
  return CUSTOMS_PARSE_STATUS_LABELS[status] || status || "未识别";
}

function stripPdfTextEscapes(value = "") {
  return String(value)
...
function extractPdfTextOperators(text = "") {
  const chunks = [];
  for (const match of String(text).matchAll(/\((?:\\.|[^\\)])*\)\s*Tj/g)) {
    chunks.push(stripPdfTextEscapes(match[0].replace(/\s*Tj$/, "").slice(1, -1)));
  }
  for (const match of String(text).matchAll(/<([0-9a-fA-F\s]+)>\s*Tj/g)) chunks.push(decodePdfHexText(match[1]));
  for (const match of String(text).matchAll(/\[(.*?)\]\s*TJ/gs)) {
    const body = match[1] || "";
    for (const literal of body.matchAll(/\((?:\\.|[^\\)])*\)/g)) chunks.push(stripPdfTextEscapes(literal[0].slice(1, -1)));
    for (const hex of body.matchAll(/<([0-9a-fA-F\s]+)>/g)) chunks.push(decodePdfHexText(hex[1]));
  }
  return chunks.join("\n");
}
...
function extractPdfTextLayer(buffer) {
  ...
  for (const match of latin.matchAll(/<<(.*?)>>\s*stream\r?\n([\s\S]*?)\r?\nendstream/g)) {
    ...
    if (/\/FlateDecode\b/.test(dictionary)) {
      ...
      decoded = streamBuffer.toString("utf8");
    }
    if (decoded) texts.push(extractPdfTextOperators(decoded));
  }
  texts.push(extractPdfTextOperators(latin));
...
function nearbyValue(text = "", labels = [], maxLength = 80) {
...
function findNearbyDate(text = "", labels = []) {
  const value = nearbyValue(text, labels, 40);
  const date = normalizeCustomsDate(value);
  if (date) return date;
  for (const label of labels) {
    const escaped = label.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
    const after = String(text || "").match(new RegExp(`${escaped}[\\s\\S]{0,80}`, "i"))?.[0] || "";
    const fallback = normalizeCustomsDate(after);
    if (fallback) return fallback;
  }
  return "";
}

function parseCustomsDeclarationText(text = "") {
  const normalized = String(text || "").replace(/\r/g, "\n");
  const declarationNo = nearbyValue(normalized, ["报关单号", "海关编号", "预录入编号"], 40).match(/[A-Z0-9]{8,32}/i)?.[0] || "";
  const declarationDate = findNearbyDate(normalized, ["申报日期", "申报时间"]);
  const exportDate = findNearbyDate(normalized, ["出口日期", "申报出口日期", "出境日期"]);
  const port = nearbyValue(normalized, ["申报口岸", "出口口岸", "进出口岸"], 32)
    .replace(/\s*(申报日期|出口日期|申报出口日期).*$/u, "")
    .trim();
  return {
    customsDeclarationNo: declarationNo,
    customsDeclarationDate: declarationDate,
    customsExportDate: exportDate,
    customsPort: port,
  };
}

async function ocrCustomsPdf(buffer, document = {}) {
  const endpoint = process.env.CUSTOMS_OCR_ENDPOINT || process.env.OCR_ENDPOINT || "";
  if (!endpoint) throw codedError("PDF 无可读取文本层，OCR 服务未配置。", 422, "CUSTOMS_OCR_NOT_CONFIGURED");
  const response = await fetch(endpoint, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({
      fileName: document.fileName || "customs-entry-form.pdf",
      mimeType: document.mimeType || "application/pdf",
      fileBase64: Buffer.from(buffer).toString("base64"),
    }),
  });
  const data = await response.json().catch(() => ({}));
  if (!response.ok) throw codedError(data.message || data.error || "OCR 服务识别失败。", response.status, data.code || "CUSTOMS_OCR_FAILED");
...
function normalizedTradeTerm(value = "") {
  return String(value || "").trim().toUpperCase();
}

function isSeaFreightRequirement(requirement = {}) {
  return requirement.key === SEA_FREIGHT_REQUIREMENT_KEY;
}

function isSeaFreightRequiredByTradeTerm(order = {}) {
  return SEA_FREIGHT_REQUIRED_TRADE_TERMS.includes(normalizedTradeTerm(order.tradeTerm));
}

function taxRefundLogisticsInvoiceRequirementsForOrder(order = {}, logisticsInvoiceCosts = []) {
...
function confirmedCost(cost) {
  return validCost(cost) && cost.costConfirmed === true;
}

function paidConfirmedCost(cost) {
...
function allCostsConfirmed(costs = []) {
...
export async function saveUser(request, actor, input, id = null) {
  ...
  if (role === LOGISTICS_SUPPLIER_ROLE) {
    const supplierId = nonEmpty(input.supplierId);
    if (!supplierId) throw codedError("物流供应商账号必须关联供应商。", 400, "SUPPLIER_REQUIRED");
    const supplier = await assertSupplierActive(supplierId);
    if (!TAX_REFUND_LOGISTICS_INVOICE_SUPPLIER_TYPES.includes(supplier.supplierType)) {
      throw codedError("物流供应商账号只能关联物流、报关、海运或港杂费用供应商。", 400, "SUPPLIER_TYPE_INVALID");
    }
...
function canAccessDomesticLogisticsOrder(actor, order) {
  ...
  if (isExternalLogisticsSupplierAccount(actor)) {
    return (order.domesticLogisticsInfos || []).some((info) => !info.deletedAt && info.submittedByUserId === actor.id);
  }
  if (actor?.role === "业务员") return order.createdById === actor.id || order.salespersonUserId === actor.id;
  return false;
}

function canClaimDomesticLogisticsOrder(actor, order = {}, input = {}) {
...
function isInternalLogisticsOperator(actor) {
  return actor?.role === LOGISTICS_OPERATOR_ROLE && !actor.supplierId;
}

function isExternalLogisticsSupplierAccount(actor) {
  return (actor?.role === LOGISTICS_OPERATOR_ROLE || actor?.role === LOGISTICS_SUPPLIER_ROLE) && Boolean(actor.supplierId);
}

function isLogisticsSupplierExpenseInternalReviewer(actor) {
  return ["管理员", "财务", "成本录入员"].includes(actor?.role);
}

function canReviewLogisticsSupplierExpense(actor) {
  return isLogisticsSupplierExpenseInternalReviewer(actor) && canWrite(actor, "logisticsExpenses");
}
...
export async function searchReceivableOrders(query, actor) {
  ...
  const accessWhere = isCostEntrySearch
    ? {}
    : orderAccessWhere(actor);
  const filters = [
    accessWhere,
...
export async function saveOrder(request, actor, input, id = null) {
  ...
  const data = {
    ...
    customerNameSnapshot: before && before.customerId === customer.id
      ? before.customerNameSnapshot
...
export async function savePayment(request, actor, input, id = null) {
  ...
  const auditAction = id && before?.status !== requestedStatus
    ? `修改收款状态：${before.status}→${requestedStatus}`
    : (id ? "更新收款" : "新增收款");
...
function orderArchiveWhereForScope(scope = "current") {
...
function includeLogisticsSupplierExpenseRelations() {
  return {
    order: { include: { customer: true, salesperson: true } },
    supplier: true,
    orderCost: { include: includeCostRelations() },
    createdBy: true,
    updatedBy: true,
    reviewedBy: true,
    documents: {
...
function logisticsSupplierExpenseAccessWhere(actor) {
  if (!canRead(actor, "logisticsExpenses")) return { id: "__no_logistics_expense_access__" };
  if (isLogisticsSupplierExpenseInternalReviewer(actor)) return {};
  if (actor?.role === "业务员") return { order: { is: { OR: [{ createdById: actor.id }, { salespersonUserId: actor.id }] } } };
  if (actor?.role === LOGISTICS_SUPPLIER_ROLE && actor.supplierId) {
    return { supplierId: actor.supplierId };
  }
  return { id: "__no_logistics_expense_access__" };
}

function logisticsSupplierAssignedOrderWhere(actor) {
  if (isLogisticsSupplierExpenseInternalReviewer(actor)) return {};
  if (actor?.role === LOGISTICS_SUPPLIER_ROLE && actor.supplierId) {
    return {
      OR: [
        { logisticsSupplierExpenses: { some: { supplierId: actor.supplierId, deletedAt: null } } },
        { costs: { some: { supplierId: actor.supplierId, deletedAt: null } } },
      ],
    };
  }
  if (actor?.role === "业务员") return { OR: [{ createdById: actor.id }, { salespersonUserId: actor.id }] };
  return { id: "__no_order_access__" };
}

function canAccessLogisticsSupplierExpense(actor, expense) {
  if (!canRead(actor, "logisticsExpenses")) return false;
  if (isLogisticsSupplierExpenseInternalReviewer(actor)) return true;
  if (actor?.role === "业务员") return canAccessOrder(actor, expense.order || {});
  if (actor?.role === LOGISTICS_SUPPLIER_ROLE) return Boolean(actor.supplierId && expense.supplierId === actor.supplierId);
  return false;
}

function assertCanModifyLogisticsSupplierExpense(actor, expense) {
  if (isLogisticsSupplierExpenseInternalReviewer(actor)) return;
  if (actor?.role === LOGISTICS_SUPPLIER_ROLE && actor.supplierId === expense.supplierId && ["草稿", "已退回"].includes(expense.reviewStatus)) return;
  throw permissionError("当前状态或角色不能修改该物流费用");
}

function assertLogisticsSupplierExpenseName(input = {}) {
  const expenseType = nonEmpty(input.expenseType);
  const customExpenseName = nonEmpty(input.customExpenseName);
  if (expenseType === "自定义费用" && !customExpenseName) {
    throw codedError("自定义费用名称不能为空。", 400, "CUSTOM_EXPENSE_NAME_REQUIRED");
  }
  if (expenseType === "其他物流费用" && !customExpenseName) {
    throw codedError("其他物流费用建议填写具体费用名称。", 400, "CUSTOM_EXPENSE_NAME_REQUIRED");
  }
  if (customExpenseName && customExpenseName.length > 60) {
    throw codedError("自定义费用名称不能超过 30 个中文字符或 60 个英文字符。", 400, "CUSTOM_EXPENSE_NAME_TOO_LONG");
  }
  return customExpenseName;
}

async function assertLogisticsExpenseOrder(orderId, actor) {
  const order = await prisma.receivableOrder.findFirst({
    where: {
      id: requireText(orderId, "关联订单"),
      deletedAt: null,
      status: { notIn: ["已取消", "已关闭"] },
      taxArchived: false,
      ...logisticsSupplierAssignedOrderWhere(actor),
    },
    include: { customer: true, costs: { where: { deletedAt: null }, select: { supplierId: true, createdById: true } }, logisticsSupplierExpenses: { where: { deletedAt: null }, select: { supplierId: true } } },
  });
  if (!order) throw codedError("订单不存在或未分配给当前供应商。", 404, "ORDER_NOT_FOUND");
  return order;
}

async function resolveLogisticsExpenseSupplier(input, actor) {
  const supplierId = actor?.role === LOGISTICS_SUPPLIER_ROLE
    ? actor.supplierId
    : nonEmpty(input.supplierId);
  if (!supplierId) throw codedError("请选择物流供应商。", 400, "SUPPLIER_REQUIRED");
  const supplier = await assertSupplierActive(supplierId);
  if (!TAX_REFUND_LOGISTICS_INVOICE_SUPPLIER_TYPES.includes(supplier.supplierType)) {
    throw codedError("物流费用登记仅支持物流、报关、海运、港杂费用供应商。", 400, "SUPPLIER_TYPE_INVALID");
  }
  return supplier;
}

async function buildLogisticsSupplierExpenseData(input, actor, before = null) {
  const order = await assertLogisticsExpenseOrder(input.orderId, actor);
  const supplier = await resolveLogisticsExpenseSupplier(input, actor);
  if (actor?.role === LOGISTICS_SUPPLIER_ROLE && supplier.id !== actor.supplierId) {
    throw permissionError("只能为当前登录供应商录入费用");
  }
  const expenseType = LOGISTICS_SUPPLIER_EXPENSE_TYPES.includes(nonEmpty(input.expenseType)) ? nonEmpty(input.expenseType) : "其他物流费用";
  const customExpenseName = assertLogisticsSupplierExpenseName({ ...input, expenseType });
  const amount = requirePositive(input.amount, "费用金额");
  ...
  const exchangeInput = currency === "CNY"
    ? { ...input, currency: "CNY", exchangeRate: 1, exchangeRateSource: "系统", exchangeRateDate: input.exchangeRateDate || input.expenseDate || todayInputInChina() }
    : input;
  const expenseDate = dateFromInput(input.expenseDate) || dateFromInput(todayInputInChina());
  const settlementMonth = nonEmpty(input.settlementMonth) || dateToInput(expenseDate).slice(0, 7);
  if (!/^\d{4}-\d{2}$/.test(settlementMonth)) throw codedError("月结月份格式应为 YYYY-MM。", 400, "SETTLEMENT_MONTH_INVALID");
  const exchange = await resolveExchangeRateSnapshot(exchangeInput, actor, {
    currency,
    defaultDate: expenseDate,
    allowHistoricalSource: before?.exchangeRateSource === "历史录入",
  });
  const taxDocumentType = LOGISTICS_SUPPLIER_EXPENSE_TAX_DOCUMENT_TYPES.includes(input.taxDocumentType)
  ...
  return {
    ...
    expenseDate,
    settlementMonth,
    invoiceStatus: LOGISTICS_SUPPLIER_EXPENSE_INVOICE_STATUSES.includes(input.invoiceStatus) ? input.invoiceStatus : (before?.invoiceStatus || "未上传"),
    paymentStatus: LOGISTICS_SUPPLIER_EXPENSE_PAYMENT_STATUSES.includes(input.paymentStatus) ? input.paymentStatus : (before?.paymentStatus || "未付款"),
    reviewStatus: before?.reviewStatus === "已退回" ? "草稿" : (before?.reviewStatus || "草稿"),
    rejectReason: null,
    remark: optional(input.remark),
    updatedById: actor.id,
...
export async function listLogisticsSupplierExpenseOrders(query, actor) {
  assertRead(actor, "logisticsExpenses");
  const keyword = nonEmpty(query.get("q") || query.get("keyword"));
  const where = {
    deletedAt: null,
    status: { notIn: ["已取消", "已关闭"] },
    ...orderArchiveWhereForScope(archiveScope(query)),
    ...logisticsSupplierAssignedOrderWhere(actor),
    ...(keyword ? {
      OR: [
...
export async function listLogisticsExpenseSuppliers(query, actor) {
  assertRead(actor, "logisticsExpenses");
  if (actor?.role === LOGISTICS_SUPPLIER_ROLE && actor.supplierId) {
    const supplier = await prisma.supplier.findFirst({ where: { id: actor.supplierId, deletedAt: null } });
  ...
  const rows = await prisma.supplier.findMany({
    where: {
      deletedAt: null,
      status: "启用",
      supplierType: { in: TAX_REFUND_LOGISTICS_INVOICE_SUPPLIER_TYPES },
      ...(keyword ? {
        OR: [
          { supplierName: { contains: keyword, mode: "insensitive" } },
          { invoiceTitle: { contains: keyword, mode: "insensitive" } },
...
function logisticsSupplierExpenseWhere(query, actor) {
  const keyword = insensitiveContains(query.get("keyword") || query.get("q"));
  const supplierId = nonEmpty(query.get("supplierId"));
  const expenseType = nonEmpty(query.get("expenseType"));
  const reviewStatus = nonEmpty(query.get("reviewStatus"));
  const paymentStatus = nonEmpty(query.get("paymentStatus"));
  const invoiceStatus = nonEmpty(query.get("invoiceStatus"));
  const settlementMonth = nonEmpty(query.get("settlementMonth"));
  const clauses = [
    keyword ? {
      OR: [
        { expenseType: keyword },
        { customExpenseName: keyword },
        { settlementMonth: keyword },
        { order: { is: { orderNo: keyword } } },
        { order: { is: { blNo: keyword } } },
        { order: { is: { customerNameSnapshot: keyword } } },
        { order: { is: { customer: { is: { name: keyword } } } } },
    ...
    supplierId && isLogisticsSupplierExpenseInternalReviewer(actor) ? { supplierId } : null,
    expenseType && LOGISTICS_SUPPLIER_EXPENSE_TYPES.includes(expenseType) ? { expenseType } : null,
    reviewStatus && LOGISTICS_SUPPLIER_EXPENSE_REVIEW_STATUSES.includes(reviewStatus) ? { reviewStatus } : null,
    paymentStatus && LOGISTICS_SUPPLIER_EXPENSE_PAYMENT_STATUSES.includes(paymentStatus) ? { paymentStatus } : null,
    invoiceStatus && LOGISTICS_SUPPLIER_EXPENSE_INVOICE_STATUSES.includes(invoiceStatus) ? { invoiceStatus } : null,
    settlementMonth ? { settlementMonth } : null,
  ].filter(Boolean);
  return {
    deletedAt: null,
    ...logisticsSupplierExpenseAccessWhere(actor),
    ...(clauses.length ? { AND: clauses } : {}),
  };
}

export async function listLogisticsSupplierExpenses(query, actor = null) {
  assertRead(actor, "logisticsExpenses");
  if (query.get("view") === "orders") return { rows: await listLogisticsSupplierExpenseOrders(query, actor), total: 0, page: 1, pageSize: 30, summary: null };
  if (query.get("view") === "suppliers") return { rows: await listLogisticsExpenseSuppliers(query, actor), total: 0, page: 1, pageSize: 100, summary: null };
  const { page, pageSize } = pageParams(query, 20, 100);
  const where = logisticsSupplierExpenseWhere(query, actor);
  const [total, rows] = await Promise.all([
    prisma.logisticsSupplierExpense.count({ where }),
    prisma.logisticsSupplierExpense.findMany({
      where,
      include: includeLogisticsSupplierExpenseRelations(),
      orderBy: [{ updatedAt: "desc" }, { createdAt: "desc" }],
      skip: (page - 1) * pageSize,
      take: pageSize,
    }),
  ]);
  const allForSummary = await prisma.logisticsSupplierExpense.findMany({
    where,
    include: { supplier: true },
  });
  const summary = summarizeLogisticsSupplierExpenses(allForSummary);
  return { ...pageResult(rows.map(serializeLogisticsSupplierExpense), total, page, pageSize), summary };
}

function summarizeLogisticsSupplierExpenses(rows = []) {
  const summary = {
    totalAmountCny: 0,
    paidAmountCny: 0,
    unpaidAmountCny: 0,
  ...
  rows.filter((row) => row.reviewStatus === "已审核" && !row.deletedAt && !row.voidedAt).forEach((row) => {
    const amount = Number(row.amountCny || 0);
    const paid = row.paymentStatus === "已付款" ? amount : 0;
    const expenseName = displayLogisticsSupplierExpenseName(row);
    const supplierName = row.supplier?.supplierName || row.supplierId;
    summary.totalAmountCny += amount;
    summary.paidAmountCny += paid;
    summary.unpaidAmountCny += amount - paid;
    summary.bySupplier[supplierName] = (summary.bySupplier[supplierName] || 0) + amount;
    summary.byExpenseType[expenseName] = (summary.byExpenseType[expenseName] || 0) + amount;
    summary.byMonth[row.settlementMonth] = (summary.byMonth[row.settlementMonth] || 0) + amount;
  });
  return summary;
}

export async function getLogisticsSupplierExpense(id, actor = null) {
  assertRead(actor, "logisticsExpenses");
  const expense = await prisma.logisticsSupplierExpense.findFirst({
    where: { id, deletedAt: null, ...logisticsSupplierExpenseAccessWhere(actor) },
    include: includeLogisticsSupplierExpenseRelations(),
  });
  if (!expense) throw permissionError("物流供应商费用不存在或无权查看", 404);
  return serializeLogisticsSupplierExpense(expense);
}

export async function saveLogisticsSupplierExpense(request, actor, input, id = null) {
  assertWrite(actor, "logisticsExpenses");
  const before = id ? await prisma.logisticsSupplierExpense.findFirst({ where: { id, deletedAt: null }, include: includeLogisticsSupplierExpenseRelations() }) : null;
  if (id && !before) throw permissionError("物流费用不存在或已删除", 404);
  if (before) assertCanModifyLogisticsSupplierExpense(actor, before);
  const data = await buildLogisticsSupplierExpenseData(input, actor, before);
  const expense = id
    ? await prisma.logisticsSupplierExpense.update({ where: { id }, data, include: includeLogisticsSupplierExpenseRelations() })
    : await prisma.logisticsSupplierExpense.create({ data, include: includeLogisticsSupplierExpenseRelations() });
  await runNonCriticalTask("物流供应商费用操作日志写入", () => writeAudit(request, actor, id ? "更新物流供应商费用" : "新建物流供应商费用", "logistics_supplier_expenses", expense.id, before, expense));
  return serializeLogisticsSupplierExpense(expense);
}

export async function submitLogisticsSupplierExpense(request, actor, id) {
  assertWrite(actor, "logisticsExpenses");
  const before = await prisma.logisticsSupplierExpense.findFirst({ where: { id, deletedAt: null }, include: includeLogisticsSupplierExpenseRelations() });
  if (!before) throw permissionError("物流费用不存在或已删除", 404);
  if (!(actor?.role === LOGISTICS_SUPPLIER_ROLE && actor.supplierId === before.supplierId) && !isLogisticsSupplierExpenseInternalReviewer(actor)) {
    throw permissionError("没有权限提交该物流费用");
  }
  if (!["草稿", "已退回"].includes(before.reviewStatus)) throw codedError("当前状态不能提交。", 400, "INVALID_REVIEW_STATUS");
  const expense = await prisma.logisticsSupplierExpense.update({
    where: { id },
    data: { reviewStatus: "已提交", rejectReason: null, updatedById: actor.id },
    include: includeLogisticsSupplierExpenseRelations(),
  });
  await runNonCriticalTask("物流供应商费用提交日志写入", () => writeAudit(request, actor, "提交物流供应商费用", "logistics_supplier_expenses", id, before, expense));
  return serializeLogisticsSupplierExpense(expense);
}

function logisticsExpenseCostType(expense) {
  return LOGISTICS_COST_TYPES.includes(expense.expenseType) ? expense.expenseType : "其他物流费用";
}

async function syncApprovedLogisticsExpenseToCost(tx, expense, actor) {
  const costType = logisticsExpenseCostType(expense);
  const data = {
    orderId: expense.orderId,
    supplierId: expense.supplierId,
    logisticsSupplierExpenseId: expense.id,
    customCostName: nonEmpty(expense.customExpenseName) || null,
    supplierNameSnapshot: expense.supplier?.supplierName || "",
    vendorName: expense.supplier?.supplierName || "",
    costType,
    currency: expense.currency,
    exchangeRate: expense.exchangeRate,
    amount: expense.amount,
    amountCny: expense.amountCny,
    paymentStatus: expense.paymentStatus === "已付款" ? "已支付" : "待支付",
    costConfirmed: true,
    costConfirmedAt: new Date(),
    paymentDate: expense.paymentStatus === "已付款" ? new Date() : null,
    invoiceStatus: expense.invoiceStatus === "已上传" || expense.invoiceStatus === "已审核" ? "已收到" : "未收到",
    remark: nonEmpty(expense.customExpenseName)
      ? `${expense.customExpenseName}${expense.remark ? `；${expense.remark}` : ""}`
      : (expense.remark || null),
    updatedById: actor.id,
    createdById: expense.createdById || actor.id,
  };
  const existing = expense.orderCostId
    ? await tx.orderCost.findFirst({ where: { id: expense.orderCostId, deletedAt: null } })
    : await tx.orderCost.findFirst({ where: { logisticsSupplierExpenseId: expense.id, deletedAt: null } });
  const cost = existing
    ? await tx.orderCost.update({ where: { id: existing.id }, data })
    : await tx.orderCost.create({ data });
  await tx.logisticsSupplierExpense.update({
    where: { id: expense.id },
    data: { orderCostId: cost.id },
  });
  await tx.orderDocument.updateMany({
    where: { logisticsSupplierExpenseId: expense.id, deletedAt: null },
    data: { costId: cost.id, supplierId: expense.supplierId, relatedModule: "SUPPLIER" },
  });
  return cost;
}

export async function reviewLogisticsSupplierExpense(request, actor, id, action, input = {}) {
  if (!canReviewLogisticsSupplierExpense(actor)) throw permissionError("没有权限审核物流供应商费用");
  const before = await prisma.logisticsSupplierExpense.findFirst({ where: { id, deletedAt: null }, include: includeLogisticsSupplierExpenseRelations() });
  if (!before) throw permissionError("物流费用不存在或已删除", 404);
  let expense;
  if (action === "approve") {
    if (!["已提交", "已退回"].includes(before.reviewStatus)) throw codedError("只有已提交费用才能审核通过。", 400, "INVALID_REVIEW_STATUS");
    const taxDocumentType = LOGISTICS_SUPPLIER_EXPENSE_TAX_DOCUMENT_TYPES.includes(input.taxDocumentType)
      ? input.taxDocumentType
      : (before.taxDocumentType || "不计入退税资料");
    expense = await prisma.$transaction(async (tx) => {
      const updated = await tx.logisticsSupplierExpense.update({
        where: { id },
        data: {
          reviewStatus: "已审核",
          ...
          rejectReason: null,
          taxDocumentType,
          invoiceStatus: before.documents?.some((doc) => doc.uploadStatus === "SUCCESS" && doc.documentType === "SUPPLIER_INVOICE") ? "已审核" : before.invoiceStatus,
          updatedById: actor.id,
        },
        include: includeLogisticsSupplierExpenseRelations(),
      });
      await syncApprovedLogisticsExpenseToCost(tx, updated, actor);
      return tx.logisticsSupplierExpense.findUnique({ where: { id }, include: includeLogisticsSupplierExpenseRelations() });
    });
    await runNonCriticalTask("退税资料完整度刷新", () => refreshTaxRefundCompleteness(before.orderId));
  } else if (action === "reject") {
    const rejectReason = requireText(input.rejectReason || input.reason, "退回原因");
    expense = await prisma.logisticsSupplierExpense.update({
      where: { id },
      data: { reviewStatus: "已退回", rejectReason, reviewedById: actor.id, reviewedAt: new Date(), updatedById: actor.id },
      include: includeLogisticsSupplierExpenseRelations(),
    });
  } else if (action === "void") {
    const voidReason = requireText(input.voidReason || input.reason, "作废原因");
    expense = await prisma.$transaction(async (tx) => {
      const updated = await tx.logisticsSupplierExpense.update({
        where: { id },
        data: { reviewStatus: "已作废", voidedAt: new Date(), voidReason, updatedById: actor.id },
        include: includeLogisticsSupplierExpenseRelations(),
      });
      if (updated.orderCostId) {
        await tx.orderCost.update({ where: { id: updated.orderCostId }, data: { deletedAt: new Date(), paymentStatus: "已取消", updatedById: actor.id } }).catch(() => null);
      }
      return updated;
    });
    await runNonCriticalTask("退税资料完整度刷新", () => refreshTaxRefundCompleteness(before.orderId));
    ...
    expense = await prisma.$transaction(async (tx) => {
      const updated = await tx.logisticsSupplierExpense.update({
        where: { id },
        data: { paymentStatus: "已付款", paidAt: new Date(), updatedById: actor.id },
        include: includeLogisticsSupplierExpenseRelations(),
      });
      const costId = updated.orderCostId || updated.orderCost?.id;
      if (costId) await tx.orderCost.update({ where: { id: costId }, data: { paymentStatus: "已支付", paymentDate: new Date(), updatedById: actor.id } }).catch(() => null);
      return updated;
    });
  } else {
    throw codedError("请选择有效审核动作。", 400, "INVALID_ACTION");
  }
  const actionLabel = { approve: "审核通过物流供应商费用", reject: "退回物流供应商费用", void: "作废物流供应商费用", paid: "标记物流供应商费用已付款" }[action];
  await runNonCriticalTask("物流供应商费用审核日志写入", () => writeAudit(request, actor, actionLabel, "logistics_supplier_expenses", id, before, expense));
  return serializeLogisticsSupplierExpense(expense);
}

export async function uploadLogisticsSupplierExpenseDocument(request, actor, id, { file, logisticsDocumentType = "发票" }) {
  assertWrite(actor, "logisticsExpenses");
  const before = await prisma.logisticsSupplierExpense.findFirst({ where: { id, deletedAt: null }, include: includeLogisticsSupplierExpenseRelations() });
  if (!before) throw permissionError("物流费用不存在或已删除", 404);
  if (!isLogisticsSupplierExpenseInternalReviewer(actor) && !(actor?.role === LOGISTICS_SUPPLIER_ROLE && actor.supplierId === before.supplierId && ["草稿", "已退回", "已提交"].includes(before.reviewStatus))) {
    throw permissionError("没有权限上传该费用附件");
  }
  ...
  const finalLogisticsDocumentType = LOGISTICS_SUPPLIER_EXPENSE_DOCUMENT_TYPES.includes(logisticsDocumentType) ? logisticsDocumentType : "发票";
  const documentType = ["发票", "报关费发票", "拖车费发票", "港杂费发票"].includes(finalLogisticsDocumentType)
    ? "SUPPLIER_INVOICE"
    : "SUPPLIER_PURCHASE_CONTRACT";
  const storageFileName = safeFileName(`${before.order?.orderNo || before.orderId}_LOGISTICS_EXPENSE_${Date.now()}_${crypto.randomUUID().slice(0, 8)}.pdf`);
  const storageKey = buildOrderDocumentKey({
    orderId: before.orderId,
    documentType: finalLogisticsDocumentType,
  ...
  try {
    document = await prisma.orderDocument.create({
      data: {
        orderId: before.orderId,
        costId: before.orderCostId || before.orderCost?.id || null,
        supplierId: before.supplierId,
        logisticsSupplierExpenseId: before.id,
        relatedModule: "LOGISTICS_EXPENSE",
        documentType,
        logisticsDocumentType: finalLogisticsDocumentType,
      ...
      include: { order: { include: { customer: true } }, cost: { include: { supplier: true } }, supplier: true, uploadedBy: true, logisticsSupplierExpense: true },
    });
    await prisma.logisticsSupplierExpense.update({
      where: { id },
      data: { invoiceStatus: documentType === "SUPPLIER_INVOICE" ? "已上传" : before.invoiceStatus, updatedById: actor.id },
    });
  } catch (error) {
    await deleteR2Object(storageKey).catch(() => null);
    throw error;
  }
  await runNonCriticalTask("物流供应商费用附件上传日志写入", () => writeAudit(request, actor, "上传物流供应商费用附件", "order_documents", document.id, null, {
    orderNo: before.order?.orderNo,
    expenseId: before.id,
    fileName: document.fileName,
    logisticsDocumentType: finalLogisticsDocumentType,
  }));
  if (before.reviewStatus === "已审核") {
    await runNonCriticalTask("成本发票状态同步", () => syncCostInvoiceStatus(before.orderCostId || before.orderCost?.id));
    await runNonCriticalTask("退税资料完整度刷新", () => refreshTaxRefundCompleteness(before.orderId));
  }
...
function domesticLogisticsRemark(input = {}) {
  ...
  const destinationPlace = requireText(input.destinationPlace, "到达地");
  const cargoDescription = requireText(input.cargoDescription, "运输货物名称");
...
export async function listDomesticLogisticsOrders(query, actor) {
  ...
  if (isExternalLogisticsSupplierAccount(actor)) {
    andConditions.push({ domesticLogisticsInfos: { some: { submittedByUserId: actor.id, deletedAt: null } } });
  }
  ...
  return orders.filter((order) => (
    canReadDomesticLogisticsOrder(actor, order)
...
async function getDomesticLogisticsOrderForActor(orderId, actor, input = {}) {
  ...
  if (!canReadDomesticLogisticsOrder(actor, order) && !canClaimDomesticLogisticsOrder(actor, order, input)) {
    throw codedError("无权限访问该订单国内物流信息", 403, "PERMISSION_DENIED");
  }
  return order;
}
...
export async function saveDomesticLogisticsInfo(request, actor, input, id = null) {
  ...
  const remarkText = remarkTextManualEdited ? optional(input.remarkText) : domesticLogisticsRemark({ ...input, transportType });
  const data = {
    orderId: order.id,
    transportType,
...
function canReadDocument(actor, document) {
  ...
  if (actor?.role === LOGISTICS_SUPPLIER_ROLE) {
    return document.relatedModule === "LOGISTICS_EXPENSE" && document.supplierId === actor.supplierId;
  }
...
function canModifyDocument(actor, document) {
  ...
  if (actor?.role === LOGISTICS_SUPPLIER_ROLE) return false;
...
function canManageCustomsRecognition(actor) {
  return ["管理员", "财务", "业务员"].includes(actor?.role);
}

function assertCanManageCustomsRecognition(actor) {
  if (!canManageCustomsRecognition(actor)) {
...
export async function uploadOrderDocument(request, actor, { orderId, documentType, file, costId = "", supplierId = "" }) {
  ...
  const uploadAction = documentType === "CUSTOMS_ENTRY_FORM" ? "报关单上传" : "上传文件";
...
function normalizeCustomsInput(input = {}) {
  return {
    ...
    customsDeclarationDate: normalizeCustomsDate(input.customsDeclarationDate) || "",
    customsExportDate: normalizeCustomsDate(input.customsExportDate) || "",
    customsPort: nonEmpty(input.customsPort).slice(0, 120) || null,
  };
}

function customsUpdateData(fields = {}, status = "SUCCESS", message = "") {
  return {
    ...
    customsExportDate: fields.customsExportDate ? dateFromInput(fields.customsExportDate) : null,
    customsPort: fields.customsPort || null,
...
function hasCustomsRecognitionValue(fields = {}) {
  return Boolean(fields.customsDeclarationNo || fields.customsDeclarationDate || fields.customsExportDate || fields.customsPort);
}

async function parseCustomsDocumentBuffer(buffer, document = {}) {
  let text = extractPdfTextLayer(buffer);
  let source = "PDF_TEXT";
  if (!text) {
    text = await ocrCustomsPdf(buffer, document);
    source = "OCR";
  }
  const fields = parseCustomsDeclarationText(text);
  if (!hasCustomsRecognitionValue(fields)) {
    const error = codedError("未能从报关单中识别报关单号、申报日期、出口日期或口岸。", 422, "CUSTOMS_PARSE_NO_FIELDS");
...
async function applyCustomsParseFailure(request, actor, orderId, message, code = "CUSTOMS_PARSE_FAILED", action = "自动识别失败", options = {}) {
  ...
  if (!before || (before.customsParseStatus === "MANUAL" && !allowManualFailure)) {
  ...
  const order = await prisma.receivableOrder.update({
    ...
    data: {
      ...
      customsParseMessage: message || "识别失败",
    },
    include: includeOrderRelations(),
  });
  await runNonCriticalTask("报关单识别失败日志写入", () => writeAudit(request, actor, action, "receivable_orders", orderId, {
...
async function parseAndApplyCustomsDocument(
  request,
  actor,
  document,
  buffer,
  ...
  if (before.customsParseStatus === "MANUAL" && !force) return null;
  try {
    const { fields, source } = await parseCustomsDocumentBuffer(buffer, document);
    const order = await prisma.receivableOrder.update({
      where: { id: document.orderId },
      data: customsUpdateData(fields, "SUCCESS", `识别来源：${source}`),
      include: includeOrderRelations(),
    });
    await runNonCriticalTask("报关单识别成功日志写入", () => writeAudit(request, actor, action, "receivable_orders", order.id, serializeCustomsRecognition(before), serializeCustomsRecognition(order)));
    return serializeOrder(order);
  } catch (error) {
    return applyCustomsParseFailure(request, actor, document.orderId, error?.message || "报关单识别失败", error?.code || "CUSTOMS_PARSE_FAILED", failureAction, {
      allowManualFailure,
    });
  }
}
...
export async function updateCustomsRecognition(request, actor, orderId, input = {}) {
  assertCanManageCustomsRecognition(actor);
  if (actor?.role === LOGISTICS_OPERATOR_ROLE) throw permissionError("物流资料录入员不能修改报关单识别字段");
  ...
  const order = await prisma.receivableOrder.update({
    where: { id: orderId },
    data: customsUpdateData(fields, "MANUAL", "人工修改"),
    include: includeOrderRelations(),
  });
  await runNonCriticalTask("报关单字段人工修改日志写入", () => writeAudit(request, actor, "手工修改申报日期/出口日期/报关单号", "receivable_orders", order.id, serializeCustomsRecognition(before), serializeCustomsRecognition(order)));
  return serializeOrder(order);
}

export async function reparseCustomsRecognition(request, actor, orderId, input = {}) {
  assertCanManageCustomsRecognition(actor);
  if (actor?.role === LOGISTICS_OPERATOR_ROLE) throw permissionError("物流资料录入员不能重新识别报关单信息");
  const documentType = nonEmpty(input.documentId || "CUSTOMS_ENTRY_FORM") === "CUSTOMS_ENTRY_FORM" ? "CUSTOMS_ENTRY_FORM" : nonEmpty(input.documentType || "") || "CUSTOMS_ENTRY_FORM";
  const order = await prisma.receivableOrder.findFirst({
    where: { id: orderId, deletedAt: null, ...orderAccessWhere(actor) },
  });
  if (!order) throw permissionError("应收订单不存在或无权修改", 404);
  if (documentType && documentType !== "CUSTOMS_ENTRY_FORM") {
    const error = permissionError("仅支持 CUSTOMS_ENTRY_FORM 的报关单识别", 400);
    error.code = "INVALID_DOCUMENT_TYPE";
    throw error;
  }
  if (order.customsParseStatus === "MANUAL" && input.confirmManualOverride !== true) {
  ...
  const buffer = await readR2Object(document.storageKey);
...
export async function previewCustomsRecognition(actor, orderId, input = {}) {
  assertCanManageCustomsRecognition(actor);
  if (actor?.role === LOGISTICS_OPERATOR_ROLE) throw permissionError("物流资料录入员不能重新识别报关单信息");
  const documentType = nonEmpty(input.documentType || "CUSTOMS_ENTRY_FORM");
  if (documentType && documentType !== "CUSTOMS_ENTRY_FORM") {
...
export async function updateTaxRefundStatus(request, actor, orderId, status, input = {}) {
  ...
  await runNonCriticalTask("退税状态操作日志写入", () => writeAudit(request, actor, status === "SUBMITTED" ? "提交退税并归档" : "修改退税状态", "receivable_orders", order.id, {
...
export async function cancelTaxRefundArchive(request, actor, orderId, nextStatus = "NOT_READY", input = {}) {
  ...
  await runNonCriticalTask("取消退税归档操作日志写入", () => writeAudit(request, actor, "取消归档", "receivable_orders", order.id, {
    orderNo: before.orderNo,
    taxRefundStatus: before.taxRefundStatus,
