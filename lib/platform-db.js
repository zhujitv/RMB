...
export const TAX_REFUND_LOGISTICS_INVOICE_COST_TYPES = ["国内物流费", "报关费", "港杂费", "文件费", "订舱费", "海运费", "目的港费用", "其他物流费用"];
...
export const CNY_ONLY_COST_TYPES = [...FACTORY_SUPPLIER_COST_TYPES, "国内物流费", "报关费", "港杂费", "文件费", "订舱费", "银行手续费", "样品费", "其他费用"];
export const FOREIGN_CURRENCY_COST_TYPES = ["海运费", "目的港费用", "国外佣金", "国外代理费", "其他物流费用"];
export const LEGACY_FOREIGN_CURRENCY_COST_TYPES = ["佣金"];
export const COST_TYPES = [...CNY_ONLY_COST_TYPES, ...FOREIGN_CURRENCY_COST_TYPES, "国内拖车费", "保险费", ...LEGACY_FOREIGN_CURRENCY_COST_TYPES]
  .filter((item, index, arr) => arr.indexOf(item) === index);
...
function isTaxRefundLogisticsInvoiceCost(cost) {
  ...
  const supplierType = supplierTypeForCost(cost);
  if (TAX_REFUND_LOGISTICS_INVOICE_SUPPLIER_TYPES.includes(supplierType)) return true;
  return supplierType === "其他供应商" && TAX_REFUND_LOGISTICS_INVOICE_COST_TYPES.includes(cost.costType);
}
...
function logisticsInvoiceMissingBucket(cost = {}) {
  return cost.costType === "港杂费" ? "missingPortInvoices" : "missingLogisticsInvoices";
}
...
export function taxDocumentCompleteness(order = {}) {
  ...
  logisticsInvoiceCosts.forEach((cost) => {
    ...
    if (!exists) {
      ...
      const daysSinceCostCreated = costCreatedAt ? Math.floor((Date.now() - costCreatedAt.getTime()) / 86400000) : 0;
      const invoiceLabel = logisticsInvoiceLabelForCost(cost);
      logisticsMissing.push({
        ...
        invoiceLabel,
        missingBucket: logisticsInvoiceMissingBucket(cost),
        label: `${supplierNameForCost(cost)} ${cost.costType}${invoiceLabel}`,
        reminderDue: daysSinceCostCreated >= 3,
        daysSinceCostCreated,
      });
    }
  });
  const logisticsTotal = logisticsInvoiceCosts.length;
  const logisticsCompleted = logisticsTotal - logisticsMissing.length;
  const missingLogisticsInvoices = logisticsMissing.filter((item) => item.missingBucket === "missingLogisticsInvoices");
  const missingPortInvoices = logisticsMissing.filter((item) => item.missingBucket === "missingPortInvoices");
  const missingLabels = [
  ...
  const total = TAX_EXPORT_DOCUMENT_TYPES.length + supplierTotal + logisticsTotal;
  const completed = exportCompleted + supplierCompleted + logisticsCompleted;
  const factory = {
  ...
  const logistics = {
    ...
    costs: logisticsInvoiceCosts.map((cost) => ({
      costId: cost.id,
      supplierId: cost.supplierId,
      supplierName: supplierNameForCost(cost),
      supplierType: supplierTypeForCost(cost),
      ...
      amountCny: Number(cost.amountCny || 0),
      currency: cost.currency || "CNY",
      invoiceLabel: logisticsInvoiceLabelForCost(cost),
    })),
  ...
  return {
    ...
    total,
    completed,
    missingTypes: [...exportMissing, ...supplierMissing.map((item) => item.documentType), ...logisticsMissing.map((item) => item.documentType)],
    missingLabels,
...
function emptyTaxRefundCompleteness() {
  ...
  return {
    complete: false,
    total: TAX_EXPORT_DOCUMENT_TYPES.length + supplierTotal,
    completed: 0,
    missingTypes: [],
    missingLabels: [],
...
function costFilterClauses(query) {
  ...
  return [
    ...
    costType && COST_TYPES.includes(costType) ? { costType } : null,
...
function costAmountBuckets(costs = []) {
  return costs.filter(validCost).reduce((acc, cost) => {
    ...
    else if (cost.costType === "港杂费" || supplierTypeForCost(cost) === "港杂费用供应商") acc.portCostCny += amount;
    else if (LOGISTICS_COST_TYPES.includes(cost.costType) || TAX_REFUND_LOGISTICS_INVOICE_COST_TYPES.includes(cost.costType)) acc.logisticsCostCny += amount;
