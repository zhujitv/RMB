...
function serializeCost(cost) {
  return {
    ...
    customerName: cost.order?.customer?.name || "",
...
export async function saveCustomer(request, actor, input, id = null) {
  ...
  const before = id ? await prisma.customer.findFirst({ where: { id, deletedAt: null } }) : null;
  const customer = id
...
async function ensureCustomer(input) {
  ...
  const name = requireText(input.customerName || input.customer, "客户");
...
async function ensureSalesperson(input, actor) {
  if (input.salespersonId) {
    return prisma.user.findFirst({ where: { id: input.salespersonId, isActive: true } });
  }
  const name = nonEmpty(input.salespersonName || input.salesperson);
  if (!name) return actor?.role === "业务员" ? actor : null;
  const email = `${name.replace(/\s+/g, "").toLowerCase()}@local.user`;
...
function validateDuplicateOrder(customerId, orderNo, id = null) {
...
export async function listOrders(query, actor) {
  const where = {
    deletedAt: null,
    ...(actor.role === "业务员" ? { OR: [{ createdById: actor.id }, { salespersonId: actor.id }] } : {}),
  };
...
export async function saveOrder(request, actor, input, id = null) {
  ...
  const salesperson = await ensureSalesperson(input, actor);
  const orderNo = requireText(input.orderNo, "订单号");
  const duplicate = await validateDuplicateOrder(customer.id, orderNo, id);
  if (duplicate) {
    const error = new Error("同一客户下已存在相同订单号");
    error.status = 400;
    throw error;
  }
  ...
  const createdAt = id
    ? (await prisma.receivableOrder.findUnique({ where: { id } }))?.createdAt
    : new Date();
  ...
  const before = id
    ? await prisma.receivableOrder.findUnique({ where: { id }, include: includeOrderRelations() })
    : null;
...
export async function listPayments(query, actor = null) {
  const rows = await prisma.payment.findMany({
    where: {
      deletedAt: null,
      ...(actor?.role === "业务员"
        ? { order: { is: { OR: [{ createdById: actor.id }, { salespersonId: actor.id }] } } }
        : {}),
    },
