...
function aggregate(lines: Array<{ name?: unknown; unit?: unknown; quantity?: unknown }>) {
  ...
  for (const line of lines) {
    ...
    map.set(key, (map.get(key) || new Prisma.Decimal(0)).add(quantity));
  }
  return map;
}
...
export function matchSupplierInvoiceToContract(invoice: VatInvoiceResult, contract: SupplierTaxContractDraft) {
  ...
  const contractLines = aggregate(contract.items.map((item) => ({ name: item.productName, unit: item.unit, quantity: item.quantity })));
  ...
  for (const [key, expected] of contractLines) {
    ...
    else if (!actual.eq(expected)) issues.push(`商品${name}的发票数量${actual.toString()}与合同数量${expected.toString()}不一致`);
  }
