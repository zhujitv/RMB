...
const COMPANY_NAME_PATTERN = /[\u4e00-\u9fa5A-Za-z0-9（）()·\-]{2,70}?(?:集团股份有限公司|股份有限公司|有限责任公司|集团有限公司|有限公司|公司|工厂|厂|中心|合作社|企业|商行|经营部)/g;

function companyNameCandidates(value: unknown) {
  const text = cleanText(value);
  if (!text) return [];
