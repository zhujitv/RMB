...
export function buildOrderDocumentKey(orderId, documentType, fileName, date = new Date()) {
  const timestamp = date.toISOString().replace(/[-:]/g, "").replace(/\.\d{3}Z$/, "");
  return `receivable-orders/${orderId}/documents/${documentType}/${timestamp}-${safeFileName(fileName)}`;
}
