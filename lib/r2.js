...
function r2Config() {
  ...
  if (missing.length) {
    ...
    throw storageError(
      `文件存储服务未配置，请联系管理员配置 Cloudflare R2 / S3。${bucketMissing ? "存储桶未配置。" : ""}缺少：${missing.join("、")}`,
      503,
...
function normalizeStorageError(error) {
  ...
  return storageError(`对象存储上传失败：${message || code || "未知错误"}`, 500, "STORAGE_UPLOAD_FAILED", { providerCode: code });
}
