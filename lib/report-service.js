...
const columnSets = {
  ...
  "tax-refunds": [
    ["orderNo", "订单号"], ["blNo", "提单号"], ["customerName", "客户名称"], ["currency", "币种"], ["finalReceivableAmountCny", "最终应收人民币"], ["receivedAmountCny", "已收人民币"], ["exportCompleteness", "出口资料完整度"], ["domesticLogisticsCompleteness", "国内物流信息完整度"], ["factoryCompleteness", "工厂资料完整度"], ["logisticsInvoiceCompleteness", "物流港杂资料完整度"], ["overallCompleteness", "总体完整度"], ["missingLogisticsInvoices", "缺失拖车费发票明细"], ["missingCustomsInvoices", "缺失报关费发票明细"], ["missingPortInvoices", "缺失港杂费发票明细"], ["taxRefundStatusLabel", "退税状态"], ["domesticLogisticsSupplier", "物流供应商"], ["domesticTransportType", "运输方式"], ["truckPlateNo", "车牌号"], ["departurePlace", "起运地"], ["departureDate", "起运日期"], ["expressTrackingNo", "快递单号"], ["exportInvoiceRemark", "出口发票备注"], ["domesticFinanceStatus", "财务审核状态"],
  ],
};
...
function domesticLogisticsColumns(order = {}) {
  ...
  return {
    ...
    truckPlateNo: info.trailerPlateNo ? `${info.truckPlateNo || ""}/${info.trailerPlateNo}` : (info.truckPlateNo || ""),
