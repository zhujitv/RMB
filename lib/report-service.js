...
function filterRows(rows, filters = {}) {
  ...
  const exportMonth = text(filters.exportMonth);
  ...
  return rows.filter((row) => {
    ...
    if (exportMonth && !dateOnly(row.customsExportDate).startsWith(exportMonth)) return false;
    return true;
  });
}
...
const columnSets = {
  ...
  profits: [
    ["orderNo", "订单号"], ["customerName", "客户名称"], ["salespersonName", "业务员"], ["receivableCny", "最终应收人民币"], ["receivedAmountCny", "已到账金额"], ["outstandingCny", "未收人民币"], ["totalCostCny", "总成本"], ["expectedGrossProfit", "预计毛利"], ["expectedGrossMargin", "预计毛利率"], ["realizedGrossProfit", "已实现毛利"], ["realizedGrossMargin", "已实现毛利率"], ["status", "订单状态"], ["destinationPlace", "到达地"], ["cargoDescription", "运输货物名称"],
  ],
  commissions: [
    ["orderNo", "订单号"], ["customerName", "客户名称"], ["salespersonName", "业务员"], ["commissionRate", "提成比例"], ["receivedAmountCny", "已到账收款"], ["logisticsCostCny", "物流成本"], ["commissionBaseCny", "提成基数"], ["commissionAmountCny", "提成金额"], ["commissionStatus", "提成状态"], ["commissionSettledAt", "结算时间"], ["destinationPlace", "到达地"], ["cargoDescription", "运输货物名称"],
  ],
  overdue: [
    ["orderNo", "订单号"], ["blNo", "提单号"], ["customerName", "客户名称"], ["salespersonName", "业务员"], ["dueDate", "到期日"], ["outstandingCny", "未收人民币"], ["reminderStatus", "逾期状态"], ["overdueDays", "逾期天数"], ["destinationPlace", "到达地"], ["cargoDescription", "运输货物名称"],
  ],
  "tax-refunds": [
    ["orderNo", "订单号"], ["blNo", "提单号"], ["customerName", "客户名称"], ["customsDeclarationNo", "报关单号"], ["customsDeclarationDate", "申报日期"], ["customsExportDate", "出口日期"], ["customsPort", "申报口岸"], ["currency", "币种"], ["finalReceivableAmountCny", "最终应收人民币"], ["receivedAmountCny", "已收人民币"], ["customsCompleteness", "报关资料完整度"], ["exportCompleteness", "出口资料完整度"], ["domesticLogisticsCompleteness", "国内物流信息完整度"], ["factoryCompleteness", "工厂资料完整度"], ["logisticsInvoiceCompleteness", "物流资料完整度"], ["overallCompleteness", "总体完整度"], ["missingLogisticsInvoices", "缺失拖车费发票明细"], ["missingCustomsInvoices", "缺失报关费发票明细"], ["missingPortInvoices", "缺失港杂费发票明细"], ["taxRefundStatusLabel", "退税状态"], ["domesticTransportType", "运输方式"], ["truckPlateNo", "车牌号"], ["trailerPlateNo", "挂车车牌"], ["departurePlace", "起运地"], ["destinationPlace", "到达地"], ["departureDate", "起运日期"], ["cargoDescription", "运输货物名称"], ["expressTrackingNo", "快递单号"], ["exportInvoiceRemark", "出口发票备注"], ["domesticSubmitterRole", "录入来源"], ["domesticSubmittedBy", "录入人"], ["domesticSubmittedAt", "录入时间"],
  ],
};
...
function orderToReceivable(order) {
  return {
    ...
    blNo: order.blNo || "待发货",
    customerName: displayCustomerName(order.customerName || order.customerNameSnapshot || order.customer?.name || ""),
    ...
    customsExportDate: toReportDate(order.customsExportDate),
...
function paymentToRow(payment) {
  return {
    ...payment,
    customerName: displayCustomerName(payment.customerName || payment.customer?.name || ""),
...
function costToRow(cost) {
  return {
    ...cost,
    customerName: displayCustomerName(cost.customerName || cost.customer?.name || ""),
...
function queryFilters(query) {
  return {
    ...
    exportMonth: query.get("exportMonth") || "",
