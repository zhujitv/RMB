...
const columnSets = {
  receivables: [
    ["orderNo", "订单号"], ["blNo", "提单号"], ["customerName", "客户名称"], ["salespersonName", "业务员"], ["currency", "币种"], ["finalReceivableAmount", "最终应收"], ["finalReceivableAmountCny", "最终应收人民币"], ["receivedAmountCny", "已收人民币"], ["outstandingCny", "未收人民币"], ["dueDate", "到期日"], ["status", "订单状态"],
  ],
  payments: [
  ...
  "tax-refunds": [
    ["orderNo", "订单号"], ["blNo", "提单号"], ["customerName", "客户名称"], ["currency", "币种"], ["finalReceivableAmountCny", "最终应收人民币"], ["receivedAmountCny", "已收人民币"], ["exportCompleteness", "出口资料"], ["supplierCompleteness", "供应商资料"], ["overallCompleteness", "总体完整度"], ["taxRefundStatusLabel", "退税状态"],
  ],
};
...
function orderToTaxRefund(order) {
  ...
  return {
    ...
    exportCompleteness: `${completeness.export?.completed || 0}/${completeness.export?.total || 7}`,
    supplierCompleteness: completeness.supplier?.missingFactoryCost
      ? "0/2（未录入工厂供应商）"
      : `${completeness.supplier?.completed || 0}/${Math.max(2, Number(completeness.supplier?.total || 0))}`,
